...
{
    global $db_account;
    $enrollmentServiceData = $db_account->Execute("SELECT DOA_ENROLLMENT_SERVICE.*, DOA_ENROLLMENT_MASTER.PK_USER_MASTER, DOA_ENROLLMENT_MASTER.PK_LOCATION, DOA_SERVICE_CODE.IS_GROUP FROM DOA_ENROLLMENT_SERVICE LEFT JOIN DOA_ENROLLMENT_MASTER ON DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER = DOA_ENROLLMENT_SERVICE.PK_ENROLLMENT_MASTER LEFT JOIN DOA_SERVICE_CODE ON DOA_ENROLLMENT_SERVICE.PK_SERVICE_CODE = DOA_SERVICE_CODE.PK_SERVICE_CODE WHERE DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER = " . $PK_ENROLLMENT_MASTER);
    while (!$enrollmentServiceData->EOF) {
        ...
        $IS_GROUP = $enrollmentServiceData->fields['IS_GROUP'];

        if ($IS_GROUP == 0) {
            $session_created = $db_account->Execute("SELECT SUM(DOA_SCHEDULING_CODE.UNIT) AS SESSION_CREATED FROM `DOA_APPOINTMENT_MASTER` LEFT JOIN DOA_SCHEDULING_CODE ON DOA_APPOINTMENT_MASTER.PK_SCHEDULING_CODE = DOA_SCHEDULING_CODE.PK_SCHEDULING_CODE WHERE DATE >= '2026-05-01' AND (PK_APPOINTMENT_STATUS NOT IN (6,4) || IS_CHARGED = 1) AND APPOINTMENT_TYPE = 'NORMAL' AND `PK_ENROLLMENT_SERVICE` = " . $PK_ENROLLMENT_SERVICE);
            ...
            $SESSION_CREATED_COUNT = ($group_session_created->RecordCount() > 0 && $group_session_created->fields['SESSION_CREATED'] != NULL) ? $group_session_created->fields['SESSION_CREATED'] : 0;

            $SESSION_LEFT = ($NUMBER_OF_SESSION - $SESSION_CREATED_COUNT > 0) ? $NUMBER_OF_SESSION - $SESSION_CREATED_COUNT : 0;
            $SESSION_LEFT = ($SESSION_LEFT > 0 && $SESSION_LEFT < 1) ? 1 : (int)$SESSION_LEFT;

            /* echo "SELECT DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER, DOA_APPOINTMENT_ENROLLMENT.IS_CHARGED, DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_STATUS FROM DOA_APPOINTMENT_MASTER INNER JOIN DOA_APPOINTMENT_CUSTOMER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER LEFT JOIN DOA_APPOINTMENT_ENROLLMENT ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_ENROLLMENT.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_CUSTOMER.PK_USER_MASTER = '$PK_USER_MASTER' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_MASTER = '$PK_SERVICE_MASTER' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_CODE = '$PK_SERVICE_CODE' AND DOA_APPOINTMENT_MASTER.PK_LOCATION = '$PK_LOCATION' AND DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'GROUP' AND DOA_APPOINTMENT_ENROLLMENT.PK_ENROLLMENT_MASTER = 0 AND DOA_APPOINTMENT_ENROLLMENT.PK_ENROLLMENT_SERVICE = 0 ORDER BY DOA_APPOINTMENT_MASTER.DATE ASC, DOA_APPOINTMENT_MASTER.START_TIME ASC";
            die(); */

            if ($SESSION_LEFT > 0) {
                $appointments = $db_account->Execute("SELECT DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER, DOA_APPOINTMENT_ENROLLMENT.IS_CHARGED, DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_STATUS FROM DOA_APPOINTMENT_MASTER INNER JOIN DOA_APPOINTMENT_ENROLLMENT ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_ENROLLMENT.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_ENROLLMENT.PK_USER_MASTER = '$PK_USER_MASTER' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_MASTER = '$PK_SERVICE_MASTER' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_CODE = '$PK_SERVICE_CODE' AND DOA_APPOINTMENT_MASTER.PK_LOCATION = '$PK_LOCATION' AND DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'GROUP' AND DOA_APPOINTMENT_ENROLLMENT.PK_ENROLLMENT_MASTER = 0 AND DOA_APPOINTMENT_ENROLLMENT.PK_ENROLLMENT_SERVICE = 0 ORDER BY DOA_APPOINTMENT_MASTER.DATE ASC, DOA_APPOINTMENT_MASTER.START_TIME ASC");
                while (!$appointments->EOF) {
                    if (($appointments->fields['PK_APPOINTMENT_STATUS'] != 4 && $appointments->fields['PK_APPOINTMENT_STATUS'] != 6) || $appointments->fields['IS_CHARGED'] == 1) {
                        $SESSION_LEFT -= 1;
                        if ($SESSION_LEFT < 0) {
                    ...
                    $db_account->Execute("UPDATE `DOA_APPOINTMENT_ENROLLMENT` SET `PK_ENROLLMENT_MASTER` = '$PK_ENROLLMENT_MASTER',`PK_ENROLLMENT_SERVICE` = '$PK_ENROLLMENT_SERVICE' WHERE PK_USER_MASTER = '$PK_USER_MASTER' AND PK_APPOINTMENT_MASTER = " . $appointments->fields['PK_APPOINTMENT_MASTER']);
                    $appointments->MoveNext();
                }
            }
        }
...
function getRefundReceipts($PK_ENROLLMENT_MASTER, $refund_amount)
{
    ...
    $sql = "
        ...
        SELECT *
        FROM receipt_running
        WHERE running_total - TOTAL_AMOUNT < '" . $refund_amount . "'
