...
function markAppointmentAdhoc($PK_ENROLLMENT_SERVICE)
{
    ...
    $SESSION_CREATED_COUNT = getSessionCreatedCount($PK_ENROLLMENT_SERVICE, 'NORMAL');
    ...
    if ($SESSION_CREATED_COUNT > $enrollment_session->fields['NUMBER_OF_SESSION']) {
        $last_appointment_id = $db_account->Execute("SELECT DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER FROM DOA_APPOINTMENT_MASTER WHERE PK_ENROLLMENT_SERVICE = '$PK_ENROLLMENT_SERVICE' AND (`PK_APPOINTMENT_STATUS` != 6 OR IS_CHARGED = 1) ORDER BY DOA_APPOINTMENT_MASTER.DATE DESC, DOA_APPOINTMENT_MASTER.START_TIME DESC LIMIT 1");
        ...
        $last_enrollment_appointment = $db_account->Execute("SELECT PK_APPOINTMENT_MASTER, PK_ENROLLMENT_MASTER, PK_ENROLLMENT_SERVICE FROM DOA_APPOINTMENT_MASTER WHERE PK_SERVICE_MASTER = ".$enrollment_session->fields['PK_SERVICE_MASTER']." AND (`PK_APPOINTMENT_STATUS` != 6 OR IS_CHARGED = 1) ORDER BY PK_ENROLLMENT_MASTER DESC, DOA_APPOINTMENT_MASTER.DATE ASC, DOA_APPOINTMENT_MASTER.START_TIME ASC LIMIT 1");
...
function markAdhocAppointmentNormal($PK_ENROLLMENT_MASTER)
{
    ...
    while (!$enrollmentServiceData->EOF) {
        ...
        $appointments = $db_account->Execute("SELECT DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER, DOA_APPOINTMENT_MASTER.IS_CHARGED FROM DOA_APPOINTMENT_MASTER LEFT JOIN DOA_APPOINTMENT_CUSTOMER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_CUSTOMER.PK_USER_MASTER = '$PK_USER_MASTER' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_MASTER = '$PK_SERVICE_MASTER' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_CODE = '$PK_SERVICE_CODE' AND DOA_APPOINTMENT_MASTER.PK_LOCATION = '$PK_LOCATION' AND DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'AD-HOC' ORDER BY DOA_APPOINTMENT_MASTER.DATE ASC, DOA_APPOINTMENT_MASTER.START_TIME ASC LIMIT ".$NUMBER_OF_SESSION);
...
function getSessionCreatedCount($PK_ENROLLMENT_SERVICE, $TYPE = null)
{
    global $db_account;
    if ($TYPE == 'NORMAL') {
        $session_created = $db_account->Execute("SELECT COUNT(PK_APPOINTMENT_MASTER) AS SESSION_CREATED FROM `DOA_APPOINTMENT_MASTER` WHERE (`PK_APPOINTMENT_STATUS` != 6 OR IS_CHARGED = 1) AND APPOINTMENT_TYPE = 'NORMAL' AND `PK_ENROLLMENT_SERVICE` = ".$PK_ENROLLMENT_SERVICE);
        return ($session_created->RecordCount() > 0) ? $session_created->fields['SESSION_CREATED'] : 0;
    } elseif ($TYPE == 'GROUP') {
        $group_session_created = $db_account->Execute("SELECT COUNT(DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER) AS SESSION_CREATED FROM DOA_APPOINTMENT_MASTER INNER JOIN DOA_APPOINTMENT_ENROLLMENT ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_ENROLLMENT.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'GROUP' AND DOA_APPOINTMENT_ENROLLMENT.PK_ENROLLMENT_SERVICE = ".$PK_ENROLLMENT_SERVICE);
        return ($group_session_created->RecordCount() > 0) ? $group_session_created->fields['SESSION_CREATED'] : 0;
    } else {
        return 0;
    }
}
...
function getSessionCompletedCount($PK_ENROLLMENT_SERVICE)
{
    ...
    if ($enrollmentServiceData->fields['IS_GROUP'] == 0) {
        $session_completed = $db_account->Execute("SELECT COUNT(PK_APPOINTMENT_MASTER) AS SESSION_COMPLETED FROM `DOA_APPOINTMENT_MASTER` WHERE IS_CHARGED = 1 AND APPOINTMENT_TYPE = 'NORMAL' AND `PK_ENROLLMENT_SERVICE` = ".$PK_ENROLLMENT_SERVICE);
        return ($session_completed->RecordCount() > 0) ? $session_completed->fields['SESSION_COMPLETED'] : 0;
    } elseif ($enrollmentServiceData->fields['IS_GROUP'] == 1) {
        $group_session_completed = $db_account->Execute("SELECT COUNT(DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER) AS SESSION_COMPLETED FROM DOA_APPOINTMENT_MASTER INNER JOIN DOA_APPOINTMENT_ENROLLMENT ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_ENROLLMENT.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'GROUP' AND DOA_APPOINTMENT_ENROLLMENT.IS_CHARGED = 1 AND DOA_APPOINTMENT_ENROLLMENT.PK_ENROLLMENT_SERVICE = ".$PK_ENROLLMENT_SERVICE);
        return ($group_session_completed->RecordCount() > 0) ? $group_session_completed->fields['SESSION_COMPLETED'] : 0;
    } else {
        return 0;
    }
}
...
{
    global $db_account;
    $enrollment_total_count = $db_account->Execute("SELECT SUM(`NUMBER_OF_SESSION`) AS TOTAL_SESSION FROM `DOA_ENROLLMENT_SERVICE` WHERE PRICE_PER_SESSION > 0 AND `PK_ENROLLMENT_MASTER` = '$PK_ENROLLMENT_MASTER'");

    $TOTAL_COMPLETED_SESSION = 0;
    $enrollmentServiceData = $db_account->Execute("SELECT PK_ENROLLMENT_SERVICE FROM `DOA_ENROLLMENT_SERVICE` WHERE PRICE_PER_SESSION > 0 AND `PK_ENROLLMENT_MASTER` = '$PK_ENROLLMENT_MASTER'");
    while (!$enrollmentServiceData->EOF) {
