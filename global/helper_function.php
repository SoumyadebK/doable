...
function markAppointmentPaid($PK_ENROLLMENT_SERVICE)
{
    ...
    if ($serviceCodeData->RecordCount() > 0) {
        ...
        if ($paid_session >= 1) {
            $db_account->Execute("UPDATE `DOA_APPOINTMENT_MASTER` SET `IS_PAID` = '1' WHERE APPOINTMENT_TYPE = 'NORMAL' AND PK_ENROLLMENT_SERVICE = '$PK_ENROLLMENT_SERVICE' ORDER BY DATE DESC, START_TIME DESC LIMIT $paid_session");
        }
    }
}

function adjustEnrollmentAppointment($PK_APPOINTMENT_MASTER)
{
    global $db_account;
    ...
    if ($enrollment_service_data->RecordCount() > 0) {
        ...
        if ($SESSION_CREATED_COUNT > $NUMBER_OF_SESSION) {
            ...
            $add_hoc_appointments = $db_account->Execute("SELECT DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER, DOA_APPOINTMENT_MASTER.IS_CHARGED FROM DOA_APPOINTMENT_MASTER LEFT JOIN DOA_APPOINTMENT_CUSTOMER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_CUSTOMER.PK_USER_MASTER = '$PK_USER_MASTER' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_MASTER = '$PK_SERVICE_MASTER' AND DOA_APPOINTMENT_MASTER.PK_LOCATION = '$PK_LOCATION' AND DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'AD-HOC' ORDER BY DOA_APPOINTMENT_MASTER.DATE ASC, DOA_APPOINTMENT_MASTER.START_TIME ASC LIMIT 1");
            if ($add_hoc_appointments->RecordCount() > 0) {
                $PK_APPOINTMENT_MASTER = $add_hoc_appointments->fields['PK_APPOINTMENT_MASTER'];
                ...
                $last_enrollment_appointment = $db_account->Execute("SELECT PK_APPOINTMENT_MASTER, PK_ENROLLMENT_MASTER, PK_ENROLLMENT_SERVICE FROM DOA_APPOINTMENT_MASTER LEFT JOIN DOA_ENROLLMENT_MASTER ON DOA_APPOINTMENT_MASTER.PK_ENROLLMENT_MASTER = DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER WHERE PK_SERVICE_MASTER = '$PK_SERVICE_MASTER' AND DOA_ENROLLMENT_MASTER.STATUS = 'A' AND (`PK_APPOINTMENT_STATUS` != 6 AND `PK_APPOINTMENT_STATUS` != 4) ORDER BY DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER DESC, DOA_APPOINTMENT_MASTER.DATE ASC, DOA_APPOINTMENT_MASTER.START_TIME ASC LIMIT 1");
...
function getPaidSessionCount($PK_ENROLLMENT_SERVICE)
{
    ...
    if ($TYPE == 'NORMAL') {
        $session_created = $db_account->Execute("SELECT SUM(DOA_SCHEDULING_CODE.UNIT) AS SESSION_CREATED FROM `DOA_APPOINTMENT_MASTER` LEFT JOIN DOA_SCHEDULING_CODE ON DOA_APPOINTMENT_MASTER.PK_SCHEDULING_CODE = DOA_SCHEDULING_CODE.PK_SCHEDULING_CODE WHERE (PK_APPOINTMENT_STATUS = 1 OR IS_CHARGED = 1) AND IS_PAID = 1 AND APPOINTMENT_TYPE = 'NORMAL' AND `PK_ENROLLMENT_SERVICE` = ".$PK_ENROLLMENT_SERVICE);
        return ($session_created->RecordCount() > 0 && $session_created->fields['SESSION_CREATED'] != NULL) ? $session_created->fields['SESSION_CREATED'] : 0;
    } elseif ($TYPE == 'GROUP') {
        $group_session_created = $db_account->Execute("SELECT SUM(DOA_SCHEDULING_CODE.UNIT) AS SESSION_CREATED FROM `DOA_APPOINTMENT_MASTER` LEFT JOIN DOA_SCHEDULING_CODE ON DOA_APPOINTMENT_MASTER.PK_SCHEDULING_CODE = DOA_SCHEDULING_CODE.PK_SCHEDULING_CODE INNER JOIN DOA_APPOINTMENT_ENROLLMENT ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_ENROLLMENT.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'GROUP' AND DOA_APPOINTMENT_ENROLLMENT.PK_ENROLLMENT_SERVICE = ".$PK_ENROLLMENT_SERVICE);
        return ($group_session_created->RecordCount() > 0 && $group_session_created->fields['SESSION_CREATED'] != NULL) ? $group_session_created->fields['SESSION_CREATED'] : 0;
...
function updateSessionCreatedCountGroupClass($PK_APPOINTMENT_MASTER, $PK_USER_MASTER)
{
    ...
    $serviceCodeData = $db_account->Execute("SELECT PK_ENROLLMENT_SERVICE, SESSION_CREATED, DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER FROM DOA_ENROLLMENT_SERVICE LEFT JOIN DOA_ENROLLMENT_MASTER ON DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER = DOA_ENROLLMENT_SERVICE.PK_ENROLLMENT_MASTER WHERE DOA_ENROLLMENT_MASTER.PK_USER_MASTER = $PK_USER_MASTER AND DOA_ENROLLMENT_SERVICE.PK_SERVICE_MASTER = $PK_SERVICE_MASTER AND DOA_ENROLLMENT_SERVICE.PK_SERVICE_CODE = $PK_SERVICE_CODE ORDER BY PK_ENROLLMENT_SERVICE DESC LIMIT 1");
    if ($serviceCodeData->RecordCount() > 0) {
