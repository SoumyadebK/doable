...
while (!$all_account->EOF) {
    ...
    $APPOINTMENT_DATA = $db_account->Execute("SELECT PK_APPOINTMENT_MASTER, CUSTOMER_ID, CAST(CONCAT(DATE, ' ', START_TIME) AS DATETIME) AS APPOINTMENT_TIME FROM DOA_APPOINTMENT_MASTER WHERE PK_ACCOUNT_MASTER = '$PK_ACCOUNT_MASTER' AND PK_APPOINTMENT_STATUS = 1 AND IS_REMINDER_SEND = 0 AND STATUS = 'A' HAVING APPOINTMENT_TIME <= '$REMIND_TIME'");
    while (!$APPOINTMENT_DATA->EOF) {
        ...
        try {
            ...
            $db_account->Execute("UPDATE `DOA_APPOINTMENT_MASTER` SET `IS_REMINDER_SEND` = '1' AND PK_APPOINTMENT_STATUS = 7 WHERE `DOA_APPOINTMENT_MASTER`.`PK_APPOINTMENT_MASTER` = ".$APPOINTMENT_DATA->fields['PK_APPOINTMENT_MASTER']);
        }
