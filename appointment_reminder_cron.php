...
$all_account = $db->Execute("SELECT PK_ACCOUNT_MASTER, HOUR, DB_NAME FROM `DOA_ACCOUNT_MASTER` WHERE `ACTIVE` = 1 AND APPOINTMENT_REMINDER = 1");
...
while (!$all_account->EOF) {
    ...
    $APPOINTMENT_DATA = $db1->Execute("SELECT DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER, CAST(CONCAT(DATE, ' ', START_TIME) AS DATETIME) AS APPOINTMENT_TIME, DOA_APPOINTMENT_CUSTOMER.PK_USER_MASTER AS CUSTOMER_ID FROM DOA_APPOINTMENT_MASTER RIGHT JOIN DOA_APPOINTMENT_CUSTOMER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER WHERE PK_APPOINTMENT_STATUS = 1 AND IS_REMINDER_SEND = 0 AND STATUS = 'A' HAVING APPOINTMENT_TIME <= '$REMIND_TIME'");
