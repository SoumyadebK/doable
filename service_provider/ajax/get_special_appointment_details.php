        ...
        <div class="col-12">
            ...
            <div class="row">
                <div class="col-6">
                    ...
                    <div class="col-md-12" style="margin-bottom: 15px; margin-top: 10px;">
                        <select class="multi_sumo_select" name="PK_USER[]" multiple>
                            ...
                            $row = $db->Execute("SELECT DISTINCT (DOA_USERS.PK_USER), CONCAT(DOA_USERS.FIRST_NAME, ' ', DOA_USERS.LAST_NAME) AS NAME, DOA_USERS.USER_NAME, DOA_USERS.EMAIL_ID, DOA_USERS.ACTIVE FROM DOA_USERS LEFT JOIN DOA_USER_ROLES ON DOA_USERS.PK_USER = DOA_USER_ROLES.PK_USER LEFT JOIN DOA_USER_LOCATION ON DOA_USERS.PK_USER = DOA_USER_LOCATION.PK_USER LEFT JOIN DOA_USER_MASTER ON DOA_USERS.PK_USER = DOA_USER_MASTER.PK_USER WHERE DOA_USER_ROLES.PK_ROLES IN(5) AND DOA_USERS.ACTIVE = 1 AND DOA_USERS.PK_ACCOUNT_MASTER = ".$_SESSION['PK_ACCOUNT_MASTER']);
                            while (!$row->EOF) { ?>
                ...
                <div class="col-6">
                    ...
                    <div class="col-md-12" style="margin-bottom: 15px; margin-top: 10px;">
                        <select class="PK_SCHEDULING_CODE" name="PK_SCHEDULING_CODE">
                            <option disabled selected>Select Scheduling Code</option>
                            <?php
                            $row = $db_account->Execute("SELECT PK_SCHEDULING_CODE, SCHEDULING_CODE FROM DOA_SCHEDULING_CODE WHERE ACTIVE = 1");
                            while (!$row->EOF) {
                                $selected = '';
                                ...
                                ?>
                                <option value="<?php echo $row->fields['PK_SCHEDULING_CODE']; ?>" <?php echo $selected ;?>><?php echo $row->fields['SCHEDULING_CODE']; ?></option>
                                <?php $row->MoveNext(); } ?>
                        </select>
                    </div>
                </div>
