										...
										<div class="widget-content" style="padding-top: 10px">
											<div class="row">
												<div class="span9">
													...
													<button style="float:right;margin-right:5px;" type="button" class="btn-danger" onclick="window.location.href='email.php<?=$URL?>'">Back</button>

													<? if($_GET['type'] != 'trash') { ?>
													<? if($_GET['type'] != 'draft') { ?>	
													<button style="float:right;margin-right:5px;" type="button" class="btn-danger" onclick="window.location.href='compose.php?id=<?=$_GET['id']?>&pk=<?=$PK_EMAIL?>&type=reply'">Reply</button>
													<? } ?>

													<button style="float:right;margin-right:5px;" type="button" class="btn-danger" onclick="window.location.href='compose.php?id=<?=$_GET['id']?>&pk=<?=$PK_EMAIL?>&type=forward'">Forward</button>
													<? } ?>

													<? if($_GET['type'] == 'trash') { ?>
														<button style="float:right;;margin-right:5px;" type="button" class="btn-danger" onclick="window.location.href='view_email.php?type=trash&id=<?=$_GET[id]?>&act=i'">Move To Inbox</button>
													<? } ?>
												</div>
											</div>


											<? if($_GET['type'] == 'sent' || $_GET['type'] == 'draft' || $_GET['type'] == 'starred'){
											...
											while (!$res->EOF) {
												...
												<div id="content_div_<?=$i?>"  style="border:1px dashed #000; padding:5px;border-radius: 7px;<?=$style?>" >
													...
													<? $res_att = $db->Execute("SELECT * FROM DOA_EMAIL_ATTACHMENT WHERE PK_EMAIL = '$PK_EMAIL' AND ACTIVE = 1");
													if($res_att->RecordCount() > 0){ ?>
														<u>Attachments</u><br />
														<? while (!$res_att->EOF) {  ?>
