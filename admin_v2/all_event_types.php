...
$header_text = '';
$header_data = $db->Execute("SELECT * FROM `DOA_HEADER_TEXT` WHERE ACTIVE = 1 AND HEADER_TITLE = 'Event Types page'");
...
if ($status_check == 'active') {
    $active_condition = "DOA_EVENT_TYPE.ACTIVE = 1";
} else {
    $active_condition = "DOA_EVENT_TYPE.ACTIVE = 0";
}

// Count total records
$count_query = "SELECT COUNT(*) as total 
                FROM DOA_EVENT_TYPE 
                WHERE DOA_EVENT_TYPE.PK_ACCOUNT_MASTER = " . intval($_SESSION['PK_ACCOUNT_MASTER']) . " 
                AND $active_condition";

if (!empty($search)) {
    $count_query .= " AND DOA_EVENT_TYPE.EVENT_TYPE LIKE '%" . addslashes($search) . "%'";
}
...
// Get event types for current page
$query = "SELECT * FROM DOA_EVENT_TYPE 
          WHERE DOA_EVENT_TYPE.PK_ACCOUNT_MASTER = " . intval($_SESSION['PK_ACCOUNT_MASTER']) . " 
          AND $active_condition";

if (!empty($search)) {
    $query .= " AND DOA_EVENT_TYPE.EVENT_TYPE LIKE '%" . addslashes($search) . "%'";
}

$query .= " ORDER BY DOA_EVENT_TYPE.EVENT_TYPE ASC LIMIT $offset, $per_page";
$event_types = $db_account->Execute($query);
?>
...
<body>
                ...
                <div class="main-card">
                    ...
                    <div class="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-3">
                        ...
                        <div class="d-flex gap-2">
                            <button class="btn btn-success-custom rounded-pill d-flex align-items-center gap-2" onclick="createNewEventType()">
                                <i class="bi bi-plus-lg"></i> Create New Event Type
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                        <div class="search-container">
                            ...
                            <input type="text" class="form-control search-input" placeholder="Search by event type name..." id="searchInput" value="<?= htmlspecialchars($search) ?>">
                        </div>
                    ...
                    <div class="text-muted small mb-3 d-flex align-items-center gap-2">
                        <i class="bi bi-tags"></i> <?= $total_records ?> <?= $total_records == 1 ? 'event type' : 'event types' ?>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table custom-table align-middle mb-4">
                            ...
                            <tbody>
                                ...
                                $counter = 0;
                                $row_number = $offset + 1;
                                if ($event_types && !$event_types->EOF):
                                    while (!$event_types->EOF):
                                        $PK_EVENT_TYPE = $event_types->fields['PK_EVENT_TYPE'];
                                        $event_type_name = $event_types->fields['EVENT_TYPE'];
                                        $color_code = $event_types->fields['COLOR_CODE'] ?? '#6c757d';
                                        $is_active = $event_types->fields['ACTIVE'] == 1;

                                        $initials = getEventTypeInitials($event_type_name);
                                        $bg_color = getEventTypeAvatarColor($counter);
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="action-icons">
                                                    <a href="javascript:;" onclick="editEventType(<?= $PK_EVENT_TYPE ?>);" title="Edit">
                                ...
                                if ($total_records == 0):
                                    ...
                                    <tr>
                                        <td colspan="5" class="text-center py-5">
                                            ...
                                            <p class="mt-3 text-muted">No event types found for the selected filters</p>
                                            <button class="btn btn-sm btn-outline-primary mt-2" onclick="createNewEventType()">Create your first event type</button>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <script>
        ...
        function editEventType(id) {
            window.location.href = "event_type.php?id=" + id;
        }
    ...
    <?php
    // Helper functions
    function getEventTypeInitials($name)
    {
    ...
    function getEventTypeAvatarColor($index)
    {
        $gradients = [
