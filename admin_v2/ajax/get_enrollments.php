...
$SELECTED_PK_ENROLLMENT_MASTER = (isset($_POST['PK_ENROLLMENT_MASTER'])) ? $_POST['PK_ENROLLMENT_MASTER'] : 0;
?>
<?php
...
while (!$row->EOF) {
    ...
    if ((($NUMBER_OF_SESSION - $USED_SESSION_COUNT) > 0) || ($row->fields['CHARGE_TYPE'] == 'Membership')) {
        $enrollment_options[] = [
            ...
            'FINAL_AMOUNT' => $row->fields['FINAL_AMOUNT'],
            'row_fields' => $row->fields,
        ];
    }
    $row->MoveNext();
}
$single_enrollment_visible = (count($enrollment_options) === 1 && $SELECTED_PK_ENROLLMENT_MASTER == 0);
foreach ($enrollment_options as $index => $enrollment) {
    $PK_ENROLLMENT_MASTER = $enrollment['PK_ENROLLMENT_MASTER'];
    $enrollment_name = $enrollment['enrollment_name'];
    $PACKAGE = $enrollment['PACKAGE'];
    $NUMBER_OF_SESSION = $enrollment['NUMBER_OF_SESSION'];
    $USED_SESSION_COUNT = $enrollment['USED_SESSION_COUNT'];
    $TOTAL_AMOUNT_PAID = $enrollment['TOTAL_AMOUNT_PAID'];
    $row_fields = $enrollment['row_fields'];
    ...
    <div class="form-check border rounded-2 p-2 mb-2">
        ...
        <div class="statusarea mt-1">
            <span><?= $row_fields['SERVICE_NAME'] . ' (' . $row_fields['SERVICE_CODE'] . ')' ?> : <?= $USED_SESSION_COUNT ?>/<?= $NUMBER_OF_SESSION ?></span>
        </div>
    </div>
<?php }
if ($SELECTED_PK_ENROLLMENT_MASTER == 0) { ?>
    <div class="form-check border rounded-2 p-2 mb-2">
        <label class="form-check-label">
