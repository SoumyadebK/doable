...
function saveServiceData($RESPONSE_DATA)
{
    ...
    $SERVICE_INFO_DATA['IS_SCHEDULE'] = $RESPONSE_DATA['IS_SCHEDULE'];
    $SERVICE_INFO_DATA['PK_LOCATION'] = $RESPONSE_DATA['PK_LOCATION'];
    $SERVICE_INFO_DATA['DESCRIPTION'] = $RESPONSE_DATA['DESCRIPTION'];
    if (empty($RESPONSE_DATA['PK_SERVICE_MASTER'])) {
        $SERVICE_INFO_DATA['ACTIVE'] = 1;
        $SERVICE_INFO_DATA['IS_DELETED'] = 0;
        ...
        $SERVICE_CODE_DATA['PK_LOCATION'] = $RESPONSE_DATA['PK_LOCATION'];
        $SERVICE_CODE_DATA['SERVICE_CODE'] = $RESPONSE_DATA['SERVICE_CODE'];
        $SERVICE_CODE_DATA['DESCRIPTION'] = $RESPONSE_DATA['DESCRIPTION'];
        ...
        $SERVICE_CODE_DATA['SORT_ORDER'] = $RESPONSE_DATA['SORT_ORDER'];
...
function saveEnrollmentData($RESPONSE_DATA)
{
    ...
    if (empty($RESPONSE_DATA['PK_ENROLLMENT_MASTER']) || $RESPONSE_DATA['PK_ENROLLMENT_MASTER'] == 0) {
        $account_data = $db->Execute("SELECT ENROLLMENT_ID_CHAR, ENROLLMENT_ID_NUM, MISCELLANEOUS_ID_CHAR, MISCELLANEOUS_ID_NUM FROM `DOA_LOCATION` WHERE `PK_LOCATION` = '$RESPONSE_DATA[PK_LOCATION]'");
    ...
    if (isset($RESPONSE_DATA['PK_SERVICE_MASTER']) && count($RESPONSE_DATA['PK_SERVICE_MASTER']) > 0) {
        ...
        for ($i = 0; $i < count($RESPONSE_DATA['PK_SERVICE_MASTER']); $i++) {
            ...
            $ENROLLMENT_SERVICE_DATA['PK_SERVICE_CODE'] = $RESPONSE_DATA['PK_SERVICE_MASTER'][$i];
    ...
    if (isset($RESPONSE_DATA['SERVICE_PROVIDER_ID']) && is_array($RESPONSE_DATA['SERVICE_PROVIDER_ID'])) {
...
function saveEnrollmentBillingData($RESPONSE_DATA)
{
    ...
    $LEDGER_DATA['IS_PAID'] = ($RESPONSE_DATA['PAYMENT_METHOD'] == 'One Time' && $RESPONSE_DATA['BALANCE_PAYABLE'] == 0) ? 1 : 0;
    ...
    if ($RESPONSE_DATA['PAYMENT_METHOD'] == 'One Time') {
        ...
        for ($i = 0; $i < $RESPONSE_DATA['NUMBER_OF_PAYMENT']; $i++) {
            ...
            if ($LEDGER_DATA['BILLED_AMOUNT'] > 0) {
                db_perform_account('DOA_ENROLLMENT_LEDGER', $LEDGER_DATA, 'insert');
            }
...
function saveProfileData($RESPONSE_DATA)
{
    ...
    if (empty($RESPONSE_DATA['PK_USER'])) {
        $USER_DATA['JOINING_DATE'] = empty($RESPONSE_DATA['CREATED_ON']) ? date("Y-m-d H:i") : date('Y-m-d', strtotime($RESPONSE_DATA['CREATED_ON']));
...
function getServiceProviderCount($RESPONSE_DATA)
{
    ...
    while (!$all_service_provider_details->EOF) {
        ...
        $is_followup_exist = $db_account->Execute("SELECT COUNT(*) AS record_count
                                                    FROM DOA_AUTOMATION_LOG
                                                    WHERE IS_ARCHIVE = 0
                                                    AND (
...
function refundGiftCertificate($RESPONSE_DATA)
{
    ...
    if ($check_result && $check_result->RecordCount() > 0) {
        ...
        $PK_PAYMENT_TYPE = $fields['PK_PAYMENT_TYPE'];
        ...
        $PAYMENT_DATA['PK_PAYMENT_TYPE'] = $check_result->fields['PK_PAYMENT_TYPE'];
        $PAYMENT_DATA['AMOUNT'] = $AMOUNT;
        $PAYMENT_DATA['TYPE'] = 'Refund Gift Certificate';
        $PAYMENT_DATA['PK_LOCATION'] = getPkLocation();
        $PAYMENT_DATA['NOTE'] = "Balance refunded from Gift Certificate";
...
function deleteAppointment($RESPONSE_DATA): void
{
    ...
    if ($RESPONSE_DATA['type'] == 'normal') {
        ...
        $PK_APPOINTMENT_MASTER = $RESPONSE_DATA['PK_APPOINTMENT_MASTER'];
        $STANDING_ID = $RESPONSE_DATA['type'];
        if ($STANDING_ID > 0) {
...
function moveToWallet($RESPONSE_DATA): void
{
    ...
    if ($PK_PAYMENT_TYPE == 7) {
        ...
        if ($payment_data->fields['PK_CUSTOMER_WALLET'] > 0) {
...
function deleteWalletPayment($RESPONSE_DATA)
{
    ...
    $wallet_data = $db_account->Execute("SELECT * FROM DOA_CUSTOMER_WALLET WHERE CUSTOMER_WALLET_PARENT = '$PK_CUSTOMER_WALLET'");
    $DELETE_IDS = [$PK_CUSTOMER_WALLET];
    while (!$wallet_data->EOF) {
        $DELETE_IDS[] = $wallet_data->fields['PK_CUSTOMER_WALLET'];
        $wallet_data->MoveNext();
    }

    foreach ($DELETE_IDS as $id) {
        $db_account->Execute("DELETE FROM `DOA_CUSTOMER_WALLET` WHERE `PK_CUSTOMER_WALLET` = '$id'");
        $db_account->Execute("DELETE FROM `DOA_ENROLLMENT_PAYMENT` WHERE `PK_CUSTOMER_WALLET` = '$id'");
    }
    echo 1;
}
...
function deleteActiveEnrollmentData($RESPONSE_DATA)
{
    ...
    $PK_ENROLLMENT_MASTER = $RESPONSE_DATA['PK_ENROLLMENT_MASTER'];
    $delete_type = $RESPONSE_DATA['delete_type'];
    ...
    if ($delete_type == 1) {
        ...
        $appointment_data = $db_account->Execute("SELECT PK_APPOINTMENT_MASTER FROM DOA_APPOINTMENT_MASTER WHERE PK_ENROLLMENT_MASTER = " . $PK_ENROLLMENT_MASTER);
        $appointment_id = [];
        while (!$appointment_data->EOF) {
            $appointment_id[] = $appointment_data->fields['PK_APPOINTMENT_MASTER'];
            $appointment_data->MoveNext();
        }

        $db_account->Execute("UPDATE DOA_APPOINTMENT_MASTER SET PK_ENROLLMENT_MASTER = 0, PK_ENROLLMENT_SERVICE = 0, APPOINTMENT_TYPE = 'AD-HOC' WHERE PK_ENROLLMENT_MASTER = " . $PK_ENROLLMENT_MASTER);

        foreach ($appointment_id as $id) {
            adjustEnrollmentAppointment($id);
        }
    }
...
function addCustomerToGroupClassProcess($RESPONSE_DATA)
{
    global $db;
    global $db_account;
    $PK_APPOINTMENT_MASTER = $RESPONSE_DATA['PK_APPOINTMENT_MASTER'];
    ...
    if ($RESPONSE_DATA['PK_ENROLLMENT_MASTER'] == 'AD-HOC') {
    ...
    if (!isset($RESPONSE_DATA['PK_ENROLLMENT_MASTER']) || $RESPONSE_DATA['PK_ENROLLMENT_MASTER'] == '') {
        ...
        if ($customer_already_added->RecordCount() > 0) {
            ...
            $partner_text = ' without partner';
            if ($RESPONSE_DATA['WITH_PARTNER'] == 1) {
                $PARTNER_GROUP_CLASS_CUSTOMER_DATA['PK_APPOINTMENT_MASTER'] = $PK_APPOINTMENT_MASTER;
                $PARTNER_GROUP_CLASS_CUSTOMER_DATA['PK_USER_MASTER'] = $PK_USER_MASTER;
                $PARTNER_GROUP_CLASS_CUSTOMER_DATA['IS_PARTNER'] = 1;
                db_perform_account('DOA_APPOINTMENT_CUSTOMER', $PARTNER_GROUP_CLASS_CUSTOMER_DATA, 'insert');
                $partner_text = ' with partner';
            ...
            $details = '(' . $user_data->fields['NAME'] . ' Added By ' . $_SESSION['FIRST_NAME'] . ' ' . $_SESSION['LAST_NAME'] . ' at ' . date("m/d/Y h:i A") . $partner_text . ')';
...
function removeCustomerFromGroupClass($RESPONSE_DATA)
{
    ...
    $db_account->Execute("DELETE FROM `DOA_APPOINTMENT_CUSTOMER` WHERE `PK_APPOINTMENT_MASTER` = '$PK_APPOINTMENT_MASTER' AND `PK_USER_MASTER` = '$PK_USER_MASTER'");
...
function mailAgreementToCustomer($RESPONSE_DATA)
{
    global $db;
    global $db_account;
    global $upload_path;
    ...
    $PK_ENROLLMENT_MASTER = $RESPONSE_DATA['enrollment_id'];
    ...
    $agreementFilePath = $http_path . $upload_path . '/enrollment_pdf/' . $AGREEMENT_PDF_LINK;
    ...
    $PK_LOCATION = $enrollment_data->fields['PK_LOCATION'];
    $location_data = $db->Execute("SELECT * FROM `DOA_LOCATION` WHERE `PK_LOCATION` = '$PK_LOCATION'");
    $location_name = $location_data->fields['LOCATION_NAME'];

    // Fetch user details
    $user_data = $db->Execute("SELECT DOA_USERS.EMAIL_ID, DOA_USERS.FIRST_NAME, DOA_USERS.LAST_NAME FROM DOA_USERS LEFT JOIN DOA_USER_MASTER ON DOA_USERS.PK_USER = DOA_USER_MASTER.PK_USER WHERE DOA_USER_MASTER.PK_USER_MASTER = '$PK_USER_MASTER'");
    $user_email = $user_data->fields['EMAIL_ID'];
    $user_first_name = $user_data->fields['FIRST_NAME'];
    $user_last_name = $user_data->fields['LAST_NAME'];
    ...
    $To = $user_email;
    $Subject = "Agreement Document for " . $enrollment_title;
                          ...
                          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff; border-radius:8px; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,0.08);">
                            ...
                            <tr>
                              <td style="background-color:#39b54a; padding:24px 32px;">
                                <h1 style="margin:0; color:#ffffff; font-size:20px; font-weight:600;">Agreement Document</h1>
                            ...
                            <tr>
                              <td style="padding:32px;">
                                <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                                  <tr>
                                    <td style="padding:10px 0; border-bottom:1px solid #eef0f4; color:#111827; font-size:14px; font-weight:600;">Click the button below to download the agreement document.</td>
                                  </tr>
                                  <tr>
                                    <td style="padding:10px 0; border-bottom:1px solid #eef0f4; color:#111827; font-size:14px; font-weight:600;">
                                      <a href="' . htmlspecialchars($agreementFilePath) . '">Download</a>
                                    </td>
                                  </tr>
                                </table>
                              </td>
                            ...
                            <tr>
                              <td style="background-color:#f9fafb; padding:16px 32px; border-top:1px solid #eef0f4;">
                                <p style="margin:0; color:#9ca3af; font-size:12px;">This agreement was sent from the ' . htmlspecialchars($location_name) . '.</p>
                              </td>
                            </tr>
    ...
    try {
        if (!$mail->send()) {
            ...
            $return_data['success'] = true;
            $return_data['message'] = 'Email sent successfully';
            echo json_encode($return_data);
        }
    } catch (phpmailerException $e) {
        $return_data['success'] = false;
...
function mailReceiptToCustomer($RESPONSE_DATA)
{
    error_reporting(E_ALL);
    ...
    $PK_ENROLLMENT_MASTER = $RESPONSE_DATA['PK_ENROLLMENT_MASTER'];
    $RECEIPT_NUMBER = $RESPONSE_DATA['RECEIPT_NUMBER'];

    $enrollment_data = $db_account->Execute("SELECT * FROM `DOA_ENROLLMENT_MASTER` WHERE `PK_ENROLLMENT_MASTER` = '$PK_ENROLLMENT_MASTER'");
    $PK_LOCATION = $enrollment_data->fields['PK_LOCATION'];

    $receipt_page = 'receipt.php?account_id=' . $_SESSION['PK_ACCOUNT_MASTER'] . '&location_id=' . $PK_LOCATION . '&master_id=' . $PK_ENROLLMENT_MASTER . '&receipt=' . $RECEIPT_NUMBER;
    $receipt_full_url = $http_path . $receipt_page; // full URL to the PDF-generating script
    ...
    $To = $user_email;
    $Subject = "Receipt for " . $RECEIPT_NUMBER;
                      ...
                      <tr>
                        <td align="center">
                          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff; border-radius:8px; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,0.08);">
                            ...
                            <tr>
                              <td style="background-color:#39b54a; padding:24px 32px;">
                                <h1 style="margin:0; color:#ffffff; font-size:20px; font-weight:600;">Enrollment Receipt</h1>
                                <p style="margin:4px 0 0; color:#c7d2fe; font-size:13px;">' . htmlspecialchars($location_name) . '</p>
                              </td>
                            </tr>
                            ...
                            <tr>
                              <td style="padding:32px;">
                                <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                                  <tr>
                                    <td style="padding:10px 0; border-bottom:1px solid #eef0f4; color:#111827; font-size:14px; font-weight:600;">Click the button below to download the receipt.</td>
                                  </tr>
                                  <tr>
                                    <td style="padding:10px 0; border-bottom:1px solid #eef0f4; color:#111827; font-size:14px; font-weight:600;">
                                      <a href="' . htmlspecialchars($receipt_full_url) . '">Download</a>
                                    </td>
                                  </tr>
                                </table>
                              </td>
                            ...
                            <tr>
                              <td style="background-color:#f9fafb; padding:16px 32px; border-top:1px solid #eef0f4;">
                                <p style="margin:0; color:#9ca3af; font-size:12px;">This receipt was sent from the ' . htmlspecialchars($location_name) . '.</p>
                              </td>
                            </tr>
