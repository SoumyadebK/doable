...
if (!empty($_GET['START_DATE'])) {
    ...
    $report_name = 'summary_of_staff_member_report';
                      ...
                      ORDER BY DU.FIRST_NAME, DU.LAST_NAME";
                                                    ...
                                                    if ($row && $row->RecordCount() > 0) {
                                                        while (!$row->EOF) {
                                                            ...
                                                            $selected = in_array($user_id, $selected_service_provider) ? 'selected' : '';
                                                    ?>
                                ...
                                <div class="card-body">
                                        ...
                                        <table id="myTable" class="table table-bordered" data-page-length='50'>
                                            <thead>
                                                ...
                                                <tr>
                                                    ...
                                                    <th style="width:25%; text-align: center; font-weight: normal !important">$<?= ($misc_refund_data->fields['MISC_REFUND'] > 0) ? '-' : '' ?><?= number_format($misc_refund_data->fields['MISC_REFUND'], 2) ?></th>
                                                    <th style="width:25%; text-align: center; font-weight: normal !important">$<?= ($regular_refund_data->fields['REGULAR_REFUND'] + $misc_refund_data->fields['MISC_REFUND'] > 0) ? '-' : '' ?><?= number_format($regular_refund_data->fields['REGULAR_REFUND'] + $misc_refund_data->fields['MISC_REFUND'], 2) ?></th>
                                                </tr>
                                                <tr>
                                                ...
                                                <tr>
                                                    <?php
                                                    $regular_data_yearly = $db_account->Execute("SELECT COALESCE(SUM(p.AMOUNT * (sp.SERVICE_PROVIDER_PERCENTAGE/100)),0) AS REGULAR_TOTAL FROM DOA_ENROLLMENT_PAYMENT p JOIN DOA_ENROLLMENT_MASTER m ON p.PK_ENROLLMENT_MASTER = m.PK_ENROLLMENT_MASTER JOIN $master_database.DOA_USER_MASTER um ON m.PK_USER_MASTER = um.PK_USER_MASTER JOIN $master_database.DOA_USERS u ON um.PK_USER = u.PK_USER JOIN DOA_ENROLLMENT_SERVICE_PROVIDER sp ON sp.PK_ENROLLMENT_MASTER = m.PK_ENROLLMENT_MASTER WHERE m.PK_ENROLLMENT_TYPE NOT IN (16,17,18) AND sp.SERVICE_PROVIDER_ID IN (" . $PK_USER . ") AND u.IS_DELETED = 0 AND m.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND p.TYPE = 'Payment' AND p.PAYMENT_DATE BETWEEN $net_year_date_condition");
                                                    $other_payment_data_yearly = $db_account->Execute("SELECT COALESCE(SUM(p.AMOUNT * (sp.SERVICE_PROVIDER_PERCENTAGE/100)),0) AS OTHER_TOTAL FROM DOA_ENROLLMENT_PAYMENT p JOIN DOA_ENROLLMENT_SERVICE_PROVIDER sp ON sp.PK_ENROLLMENT_MASTER = p.PK_ENROLLMENT_MASTER WHERE p.PK_ENROLLMENT_MASTER = 0 AND p.TYPE = 'Payment' AND sp.SERVICE_PROVIDER_ID IN (" . $PK_USER . ") AND p.PAYMENT_DATE BETWEEN $net_year_date_condition");
                                                    $misc_data_yearly = $db_account->Execute("SELECT COALESCE(SUM(p.AMOUNT * (sp.SERVICE_PROVIDER_PERCENTAGE/100)),0) AS MISC_TOTAL FROM DOA_ENROLLMENT_PAYMENT p JOIN DOA_ENROLLMENT_MASTER m ON p.PK_ENROLLMENT_MASTER = m.PK_ENROLLMENT_MASTER JOIN $master_database.DOA_USER_MASTER um ON m.PK_USER_MASTER = um.PK_USER_MASTER JOIN $master_database.DOA_USERS u ON um.PK_USER = u.PK_USER JOIN DOA_ENROLLMENT_SERVICE_PROVIDER sp ON sp.PK_ENROLLMENT_MASTER = m.PK_ENROLLMENT_MASTER WHERE m.PK_ENROLLMENT_TYPE IN (16,17) AND sp.SERVICE_PROVIDER_ID IN (" . $PK_USER . ") AND u.IS_DELETED = 0 AND m.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND p.TYPE = 'Payment' AND p.PAYMENT_DATE BETWEEN $net_year_date_condition");
                                                    ?>
                                                ...
                                                <tr>
                                                    <?php
                                                    $regular_data_prev_year = $db_account->Execute("SELECT COALESCE(SUM(p.AMOUNT * (sp.SERVICE_PROVIDER_PERCENTAGE/100)),0) AS REGULAR_TOTAL FROM DOA_ENROLLMENT_PAYMENT p JOIN DOA_ENROLLMENT_MASTER m ON p.PK_ENROLLMENT_MASTER = m.PK_ENROLLMENT_MASTER JOIN $master_database.DOA_USER_MASTER um ON m.PK_USER_MASTER = um.PK_USER_MASTER JOIN $master_database.DOA_USERS u ON um.PK_USER = u.PK_USER JOIN DOA_ENROLLMENT_SERVICE_PROVIDER sp ON sp.PK_ENROLLMENT_MASTER = m.PK_ENROLLMENT_MASTER WHERE (m.PK_ENROLLMENT_TYPE IS NULL OR m.PK_ENROLLMENT_TYPE = '' OR m.PK_ENROLLMENT_TYPE NOT IN (16,17,18)) AND sp.SERVICE_PROVIDER_ID IN (" . $PK_USER . ") AND u.IS_DELETED = 0 AND m.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND p.TYPE = 'Payment' AND p.PAYMENT_DATE BETWEEN $prev_year_date_condition");
                                                    $other_payment_data_prev_year = $db_account->Execute("SELECT COALESCE(SUM(p.AMOUNT * (sp.SERVICE_PROVIDER_PERCENTAGE/100)),0) AS OTHER_TOTAL FROM DOA_ENROLLMENT_PAYMENT p JOIN DOA_ENROLLMENT_SERVICE_PROVIDER sp ON sp.PK_ENROLLMENT_MASTER = p.PK_ENROLLMENT_MASTER WHERE p.PK_ENROLLMENT_MASTER = 0 AND p.TYPE = 'Payment' AND sp.SERVICE_PROVIDER_ID IN (" . $PK_USER . ") AND p.PAYMENT_DATE BETWEEN $prev_year_date_condition");
                                                    $misc_data_prev_year = $db_account->Execute("SELECT COALESCE(SUM(p.AMOUNT * (sp.SERVICE_PROVIDER_PERCENTAGE/100)),0) AS MISC_TOTAL FROM DOA_ENROLLMENT_PAYMENT p JOIN DOA_ENROLLMENT_MASTER m ON p.PK_ENROLLMENT_MASTER = m.PK_ENROLLMENT_MASTER JOIN $master_database.DOA_USER_MASTER um ON m.PK_USER_MASTER = um.PK_USER_MASTER JOIN $master_database.DOA_USERS u ON um.PK_USER = u.PK_USER JOIN DOA_ENROLLMENT_SERVICE_PROVIDER sp ON sp.PK_ENROLLMENT_MASTER = m.PK_ENROLLMENT_MASTER WHERE m.PK_ENROLLMENT_TYPE IN (16,17) AND sp.SERVICE_PROVIDER_ID IN (" . $PK_USER . ") AND u.IS_DELETED = 0 AND m.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND p.TYPE = 'Payment' AND p.PAYMENT_DATE BETWEEN $prev_year_date_condition");
                                                    ?>
                                    ...
                                    <div class="table-responsive">
                                        <table id="myTable" class="table table-bordered" data-page-length='50'>
                                            <thead>
                                                <tr>
                                                    <th style="width:20%; text-align: center; vertical-align:auto; font-weight: bold; border: 1px solid black; border-bottom: 0px solid black;" colspan="3">INQUIRIES</th>
                                                ...
                                                <tr>
                                                    <th style="width:10%; text-align: center; vertical-align:auto; font-weight: bold; border-left: 1px solid black;"></th>
                                                ...
                                                <tr>
                                                    <?php
                                                    $active_interview_renewal_data = $db_account->Execute("SELECT (SELECT CONCAT(SUM(IF(Active.PK_USER_MASTER IS NOT NULL AND (11th_date IS NULL OR Active.DATE <= 11th_date), 1, 0)), '/', SUM(IF(Active.PK_USER_MASTER IS NOT NULL AND Active.DATE > 11th_date, 1, 0))) FROM (SELECT PK_USER_MASTER, SUM(DOA_SCHEDULING_CODE.UNIT) AS lessons, SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(DOA_APPOINTMENT_MASTER.DATE ORDER BY DOA_APPOINTMENT_MASTER.DATE SEPARATOR ','), ',', 11), ',', -1) AS 11th_date FROM DOA_APPOINTMENT_CUSTOMER LEFT JOIN DOA_APPOINTMENT_MASTER ON DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER LEFT JOIN DOA_SCHEDULING_CODE ON DOA_APPOINTMENT_MASTER.PK_SCHEDULING_CODE = DOA_SCHEDULING_CODE.PK_SCHEDULING_CODE WHERE DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE != 'GROUP' GROUP BY PK_USER_MASTER HAVING lessons > 10) markers RIGHT JOIN (SELECT PK_USER_MASTER, MAX(DOA_APPOINTMENT_MASTER.DATE) AS DATE FROM DOA_APPOINTMENT_CUSTOMER LEFT JOIN DOA_APPOINTMENT_MASTER ON DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER LEFT JOIN DOA_APPOINTMENT_SERVICE_PROVIDER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_SERVICE_PROVIDER.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_SERVICE_PROVIDER.PK_USER IN (" . $PK_USER . ") AND IS_PAID = 1 AND DOA_APPOINTMENT_MASTER.IS_CHARGED = 1 AND DOA_APPOINTMENT_MASTER.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND DOA_APPOINTMENT_MASTER.DATE BETWEEN DATE_ADD('$from_date', INTERVAL -23 DAY) AND DATE_ADD('$from_date', INTERVAL 6 DAY) GROUP BY PK_USER_MASTER) Active USING (PK_USER_MASTER)) AS ACTIVE_INTERVIEW_RENEWAL_COUNT");
                                                    $active_interview_renewal_count = is_null($active_interview_renewal_data->fields['ACTIVE_INTERVIEW_RENEWAL_COUNT']) || empty($active_interview_renewal_data->fields['ACTIVE_INTERVIEW_RENEWAL_COUNT']) ? 0 : explode('/', $active_interview_renewal_data->fields['ACTIVE_INTERVIEW_RENEWAL_COUNT']);

                                                    $weekly_interview_renewal_data = $db_account->Execute("SELECT CONCAT(SUM(CASE WHEN t.11th_date IS NULL OR am.DATE <= t.11th_date THEN 1 ELSE 0 END),'/',SUM(CASE WHEN t.11th_date IS NOT NULL AND am.DATE > t.11th_date THEN 1 ELSE 0 END)) AS INTERVIEW_RENEWAL_COUNT FROM DOA_APPOINTMENT_CUSTOMER ac JOIN DOA_APPOINTMENT_MASTER am ON ac.PK_APPOINTMENT_MASTER = am.PK_APPOINTMENT_MASTER JOIN DOA_APPOINTMENT_SERVICE_PROVIDER asp ON am.PK_APPOINTMENT_MASTER = asp.PK_APPOINTMENT_MASTER JOIN (SELECT ac2.PK_USER_MASTER,NULLIF(SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(am2.DATE ORDER BY am2.DATE), ',', 11), ',', -1),'') AS 11th_date FROM DOA_APPOINTMENT_CUSTOMER ac2 JOIN DOA_APPOINTMENT_MASTER am2 ON ac2.PK_APPOINTMENT_MASTER = am2.PK_APPOINTMENT_MASTER JOIN DOA_APPOINTMENT_SERVICE_PROVIDER asp2 ON am2.PK_APPOINTMENT_MASTER = asp2.PK_APPOINTMENT_MASTER WHERE asp2.PK_USER IN (" . $PK_USER . ") AND am2.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND am2.PK_APPOINTMENT_STATUS != 6 AND am2.APPOINTMENT_TYPE IN ('NORMAL','AD-HOC') GROUP BY ac2.PK_USER_MASTER) t ON ac.PK_USER_MASTER = t.PK_USER_MASTER WHERE asp.PK_USER IN (" . $PK_USER . ") AND am.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND am.PK_APPOINTMENT_STATUS != 6 AND am.APPOINTMENT_TYPE IN ('NORMAL','AD-HOC') AND am.DATE BETWEEN $weekly_date_condition");
                                                    $weekly_interview_renewal_count = is_null($weekly_interview_renewal_data->fields['INTERVIEW_RENEWAL_COUNT']) || empty($weekly_interview_renewal_data->fields['INTERVIEW_RENEWAL_COUNT']) ? 0 : explode('/', $weekly_interview_renewal_data->fields['INTERVIEW_RENEWAL_COUNT']);
                                                    //$group_data_weekly = $db_account->Execute("SELECT COUNT(DOA_APPOINTMENT_CUSTOMER.PK_USER_MASTER) AS GROUP_COUNT FROM DOA_APPOINTMENT_CUSTOMER LEFT JOIN DOA_APPOINTMENT_MASTER ON DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER LEFT JOIN DOA_APPOINTMENT_SERVICE_PROVIDER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_SERVICE_PROVIDER.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_SERVICE_PROVIDER.PK_USER IN (" . $PK_USER . ") AND DOA_APPOINTMENT_MASTER.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'GROUP' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_MASTER != 2 AND DOA_APPOINTMENT_MASTER.IS_PAID = 1 AND DOA_APPOINTMENT_MASTER.DATE BETWEEN $weekly_date_condition");
                                                    $group_data_weekly = $db_account->Execute("SELECT COUNT(DOA_APPOINTMENT_CUSTOMER.PK_USER_MASTER) AS GROUP_COUNT FROM DOA_APPOINTMENT_CUSTOMER LEFT JOIN DOA_APPOINTMENT_MASTER ON DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER LEFT JOIN DOA_APPOINTMENT_SERVICE_PROVIDER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_SERVICE_PROVIDER.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_SERVICE_PROVIDER.PK_USER IN (" . $PK_USER . ") AND DOA_APPOINTMENT_MASTER.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'GROUP' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_MASTER != 2 AND DOA_APPOINTMENT_MASTER.DATE BETWEEN $weekly_date_condition");

                                                    $weekly_booked_data = $db_account->Execute("WITH ranked_appts AS (SELECT dac.PK_USER_MASTER, dam.PK_APPOINTMENT_MASTER, dam.DATE, ROW_NUMBER() OVER (PARTITION BY dac.PK_USER_MASTER ORDER BY dam.DATE ASC, dam.PK_APPOINTMENT_MASTER ASC) AS rn FROM DOA_APPOINTMENT_CUSTOMER dac JOIN DOA_APPOINTMENT_MASTER dam ON dac.PK_APPOINTMENT_MASTER = dam.PK_APPOINTMENT_MASTER WHERE dam.PK_APPOINTMENT_STATUS != 6 AND dam.APPOINTMENT_TYPE IN ('NORMAL','AD-HOC') AND dam.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ")) SELECT COUNT(*) AS BOOKED_COUNT FROM ranked_appts ra JOIN (SELECT dasp.PK_APPOINTMENT_MASTER FROM DOA_APPOINTMENT_SERVICE_PROVIDER dasp GROUP BY dasp.PK_APPOINTMENT_MASTER HAVING COUNT(DISTINCT dasp.PK_USER) = 1 AND MAX(dasp.PK_USER) IN (" . $PK_USER . ")) only_one ON ra.PK_APPOINTMENT_MASTER = only_one.PK_APPOINTMENT_MASTER WHERE ra.rn = 1 AND ra.DATE BETWEEN $weekly_date_condition");
                                                    $weekly_showed_data = $db_account->Execute("WITH ranked_appts AS (SELECT dac.PK_USER_MASTER, dam.PK_APPOINTMENT_MASTER, dam.DATE, ROW_NUMBER() OVER (PARTITION BY dac.PK_USER_MASTER ORDER BY dam.DATE ASC, dam.PK_APPOINTMENT_MASTER ASC) AS rn FROM DOA_APPOINTMENT_CUSTOMER dac JOIN DOA_APPOINTMENT_MASTER dam ON dac.PK_APPOINTMENT_MASTER = dam.PK_APPOINTMENT_MASTER WHERE dam.IS_CHARGED = 1 AND dam.APPOINTMENT_TYPE IN ('NORMAL','AD-HOC') AND dam.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ")) SELECT COUNT(*) AS SHOWED_COUNT FROM ranked_appts ra JOIN (SELECT dasp.PK_APPOINTMENT_MASTER FROM DOA_APPOINTMENT_SERVICE_PROVIDER dasp GROUP BY dasp.PK_APPOINTMENT_MASTER HAVING COUNT(DISTINCT dasp.PK_USER) = 1 AND MAX(dasp.PK_USER) IN (" . $PK_USER . ")) only_one ON ra.PK_APPOINTMENT_MASTER = only_one.PK_APPOINTMENT_MASTER WHERE ra.rn = 1 AND ra.DATE BETWEEN $weekly_date_condition");
                                                    ?>
                                                ...
                                                <tr>
                                                    <?php
                                                    $yearly_interview_renewal_data = $db_account->Execute("SELECT CONCAT(SUM(CASE WHEN t.11th_date IS NULL OR am.DATE <= t.11th_date THEN 1 ELSE 0 END),'/',SUM(CASE WHEN t.11th_date IS NOT NULL AND am.DATE > t.11th_date THEN 1 ELSE 0 END)) AS INTERVIEW_RENEWAL_COUNT FROM DOA_APPOINTMENT_CUSTOMER ac JOIN DOA_APPOINTMENT_MASTER am ON ac.PK_APPOINTMENT_MASTER = am.PK_APPOINTMENT_MASTER JOIN DOA_APPOINTMENT_SERVICE_PROVIDER asp ON am.PK_APPOINTMENT_MASTER = asp.PK_APPOINTMENT_MASTER JOIN (SELECT ac2.PK_USER_MASTER,NULLIF(SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(am2.DATE ORDER BY am2.DATE), ',', 11), ',', -1),'') AS 11th_date FROM DOA_APPOINTMENT_CUSTOMER ac2 JOIN DOA_APPOINTMENT_MASTER am2 ON ac2.PK_APPOINTMENT_MASTER = am2.PK_APPOINTMENT_MASTER JOIN DOA_APPOINTMENT_SERVICE_PROVIDER asp2 ON am2.PK_APPOINTMENT_MASTER = asp2.PK_APPOINTMENT_MASTER WHERE asp2.PK_USER IN (" . $PK_USER . ") AND am2.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND am2.PK_APPOINTMENT_STATUS != 6 AND am2.APPOINTMENT_TYPE IN ('NORMAL','AD-HOC') GROUP BY ac2.PK_USER_MASTER) t ON ac.PK_USER_MASTER = t.PK_USER_MASTER WHERE asp.PK_USER IN (" . $PK_USER . ") AND am.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND am.PK_APPOINTMENT_STATUS != 6 AND am.APPOINTMENT_TYPE IN ('NORMAL','AD-HOC') AND am.DATE BETWEEN $net_year_date_condition");
                                                    ...
                                                    //$group_data_yearly = $db_account->Execute("SELECT COUNT(DOA_APPOINTMENT_CUSTOMER.PK_USER_MASTER) AS GROUP_COUNT FROM DOA_APPOINTMENT_CUSTOMER LEFT JOIN DOA_APPOINTMENT_MASTER ON DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER LEFT JOIN DOA_APPOINTMENT_SERVICE_PROVIDER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_SERVICE_PROVIDER.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_SERVICE_PROVIDER.PK_USER IN (" . $PK_USER . ") AND DOA_APPOINTMENT_MASTER.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'GROUP' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_MASTER != 2 AND DOA_APPOINTMENT_MASTER.IS_PAID = 1 AND DOA_APPOINTMENT_MASTER.DATE BETWEEN $net_year_date_condition");
                                                    $group_data_yearly = $db_account->Execute("SELECT COUNT(DOA_APPOINTMENT_CUSTOMER.PK_USER_MASTER) AS GROUP_COUNT FROM DOA_APPOINTMENT_CUSTOMER LEFT JOIN DOA_APPOINTMENT_MASTER ON DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER LEFT JOIN DOA_APPOINTMENT_SERVICE_PROVIDER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_SERVICE_PROVIDER.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_SERVICE_PROVIDER.PK_USER IN (" . $PK_USER . ") AND DOA_APPOINTMENT_MASTER.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'GROUP' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_MASTER != 2 AND DOA_APPOINTMENT_MASTER.DATE BETWEEN $net_year_date_condition");

                                                    $yearly_booked_data = $db_account->Execute("WITH ranked_appts AS (SELECT dac.PK_USER_MASTER, dam.PK_APPOINTMENT_MASTER, dam.DATE, ROW_NUMBER() OVER (PARTITION BY dac.PK_USER_MASTER ORDER BY dam.DATE ASC, dam.PK_APPOINTMENT_MASTER ASC) AS rn FROM DOA_APPOINTMENT_CUSTOMER dac JOIN DOA_APPOINTMENT_MASTER dam ON dac.PK_APPOINTMENT_MASTER = dam.PK_APPOINTMENT_MASTER WHERE dam.PK_APPOINTMENT_STATUS != 6 AND dam.APPOINTMENT_TYPE IN ('NORMAL','AD-HOC') AND dam.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ")) SELECT COUNT(*) AS BOOKED_COUNT FROM ranked_appts ra JOIN (SELECT dasp.PK_APPOINTMENT_MASTER FROM DOA_APPOINTMENT_SERVICE_PROVIDER dasp GROUP BY dasp.PK_APPOINTMENT_MASTER HAVING COUNT(DISTINCT dasp.PK_USER) = 1 AND MAX(dasp.PK_USER) IN (" . $PK_USER . ")) only_one ON ra.PK_APPOINTMENT_MASTER = only_one.PK_APPOINTMENT_MASTER WHERE ra.rn = 1 AND ra.DATE BETWEEN $net_year_date_condition");
                                                    $yearly_showed_data = $db_account->Execute("WITH ranked_appts AS (SELECT dac.PK_USER_MASTER, dam.PK_APPOINTMENT_MASTER, dam.DATE, ROW_NUMBER() OVER (PARTITION BY dac.PK_USER_MASTER ORDER BY dam.DATE ASC, dam.PK_APPOINTMENT_MASTER ASC) AS rn FROM DOA_APPOINTMENT_CUSTOMER dac JOIN DOA_APPOINTMENT_MASTER dam ON dac.PK_APPOINTMENT_MASTER = dam.PK_APPOINTMENT_MASTER WHERE dam.IS_CHARGED = 1 AND dam.APPOINTMENT_TYPE IN ('NORMAL','AD-HOC') AND dam.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ")) SELECT COUNT(*) AS SHOWED_COUNT FROM ranked_appts ra JOIN (SELECT dasp.PK_APPOINTMENT_MASTER FROM DOA_APPOINTMENT_SERVICE_PROVIDER dasp GROUP BY dasp.PK_APPOINTMENT_MASTER HAVING COUNT(DISTINCT dasp.PK_USER) = 1 AND MAX(dasp.PK_USER) IN (" . $PK_USER . ")) only_one ON ra.PK_APPOINTMENT_MASTER = only_one.PK_APPOINTMENT_MASTER WHERE ra.rn = 1 AND ra.DATE BETWEEN $net_year_date_condition");
                                                    ?>
                                                ...
                                                <tr>
                                                    <?php
                                                    $prev_year_interview_renewal_data = $db_account->Execute("SELECT CONCAT(SUM(CASE WHEN t.11th_date IS NULL OR am.DATE <= t.11th_date THEN 1 ELSE 0 END),'/',SUM(CASE WHEN t.11th_date IS NOT NULL AND am.DATE > t.11th_date THEN 1 ELSE 0 END)) AS INTERVIEW_RENEWAL_COUNT FROM DOA_APPOINTMENT_CUSTOMER ac JOIN DOA_APPOINTMENT_MASTER am ON ac.PK_APPOINTMENT_MASTER = am.PK_APPOINTMENT_MASTER JOIN DOA_APPOINTMENT_SERVICE_PROVIDER asp ON am.PK_APPOINTMENT_MASTER = asp.PK_APPOINTMENT_MASTER JOIN (SELECT ac2.PK_USER_MASTER,NULLIF(SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(am2.DATE ORDER BY am2.DATE), ',', 11), ',', -1),'') AS 11th_date FROM DOA_APPOINTMENT_CUSTOMER ac2 JOIN DOA_APPOINTMENT_MASTER am2 ON ac2.PK_APPOINTMENT_MASTER = am2.PK_APPOINTMENT_MASTER JOIN DOA_APPOINTMENT_SERVICE_PROVIDER asp2 ON am2.PK_APPOINTMENT_MASTER = asp2.PK_APPOINTMENT_MASTER WHERE asp2.PK_USER IN (" . $PK_USER . ") AND am2.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND am2.PK_APPOINTMENT_STATUS != 6 AND am2.APPOINTMENT_TYPE IN ('NORMAL','AD-HOC') GROUP BY ac2.PK_USER_MASTER) t ON ac.PK_USER_MASTER = t.PK_USER_MASTER WHERE asp.PK_USER IN (" . $PK_USER . ") AND am.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND am.PK_APPOINTMENT_STATUS != 6 AND am.APPOINTMENT_TYPE IN ('NORMAL','AD-HOC') AND am.DATE BETWEEN $prev_year_date_condition");
                                                    $prev_year_interview_renewal_count = explode('/', $prev_year_interview_renewal_data->fields['INTERVIEW_RENEWAL_COUNT']);
                                                    //$group_data_prev_year = $db_account->Execute("SELECT COUNT(DOA_APPOINTMENT_CUSTOMER.PK_USER_MASTER) AS GROUP_COUNT FROM DOA_APPOINTMENT_CUSTOMER LEFT JOIN DOA_APPOINTMENT_MASTER ON DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER LEFT JOIN DOA_APPOINTMENT_SERVICE_PROVIDER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_SERVICE_PROVIDER.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_SERVICE_PROVIDER.PK_USER IN (" . $PK_USER . ") AND DOA_APPOINTMENT_MASTER.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'GROUP' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_MASTER != 2 AND DOA_APPOINTMENT_MASTER.IS_PAID = 1 AND DOA_APPOINTMENT_MASTER.DATE BETWEEN $prev_year_date_condition");
                                                    $group_data_prev_year = $db_account->Execute("SELECT COUNT(DOA_APPOINTMENT_CUSTOMER.PK_USER_MASTER) AS GROUP_COUNT FROM DOA_APPOINTMENT_CUSTOMER LEFT JOIN DOA_APPOINTMENT_MASTER ON DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER LEFT JOIN DOA_APPOINTMENT_SERVICE_PROVIDER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER = DOA_APPOINTMENT_SERVICE_PROVIDER.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_SERVICE_PROVIDER.PK_USER IN (" . $PK_USER . ") AND DOA_APPOINTMENT_MASTER.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ") AND DOA_APPOINTMENT_MASTER.APPOINTMENT_TYPE = 'GROUP' AND DOA_APPOINTMENT_MASTER.PK_SERVICE_MASTER != 2 AND DOA_APPOINTMENT_MASTER.DATE BETWEEN $prev_year_date_condition");

                                                    $prev_year_booked_data = $db_account->Execute("WITH ranked_appts AS (SELECT dac.PK_USER_MASTER, dam.PK_APPOINTMENT_MASTER, dam.DATE, ROW_NUMBER() OVER (PARTITION BY dac.PK_USER_MASTER ORDER BY dam.DATE ASC, dam.PK_APPOINTMENT_MASTER ASC) AS rn FROM DOA_APPOINTMENT_CUSTOMER dac JOIN DOA_APPOINTMENT_MASTER dam ON dac.PK_APPOINTMENT_MASTER = dam.PK_APPOINTMENT_MASTER WHERE dam.PK_APPOINTMENT_STATUS != 6 AND dam.APPOINTMENT_TYPE IN ('NORMAL','AD-HOC') AND dam.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ")) SELECT COUNT(*) AS BOOKED_COUNT FROM ranked_appts ra JOIN (SELECT dasp.PK_APPOINTMENT_MASTER FROM DOA_APPOINTMENT_SERVICE_PROVIDER dasp GROUP BY dasp.PK_APPOINTMENT_MASTER HAVING COUNT(DISTINCT dasp.PK_USER) = 1 AND MAX(dasp.PK_USER) IN (" . $PK_USER . ")) only_one ON ra.PK_APPOINTMENT_MASTER = only_one.PK_APPOINTMENT_MASTER WHERE ra.rn = 1 AND ra.DATE BETWEEN $prev_year_date_condition");
                                                    $prev_year_showed_data = $db_account->Execute("WITH ranked_appts AS (SELECT dac.PK_USER_MASTER, dam.PK_APPOINTMENT_MASTER, dam.DATE, ROW_NUMBER() OVER (PARTITION BY dac.PK_USER_MASTER ORDER BY dam.DATE ASC, dam.PK_APPOINTMENT_MASTER ASC) AS rn FROM DOA_APPOINTMENT_CUSTOMER dac JOIN DOA_APPOINTMENT_MASTER dam ON dac.PK_APPOINTMENT_MASTER = dam.PK_APPOINTMENT_MASTER WHERE dam.IS_CHARGED = 1 AND dam.APPOINTMENT_TYPE IN ('NORMAL','AD-HOC') AND dam.PK_LOCATION IN (" . $_SESSION['DEFAULT_LOCATION_ID'] . ")) SELECT COUNT(*) AS SHOWED_COUNT FROM ranked_appts ra JOIN (SELECT dasp.PK_APPOINTMENT_MASTER FROM DOA_APPOINTMENT_SERVICE_PROVIDER dasp GROUP BY dasp.PK_APPOINTMENT_MASTER HAVING COUNT(DISTINCT dasp.PK_USER) = 1 AND MAX(dasp.PK_USER) IN (" . $PK_USER . ")) only_one ON ra.PK_APPOINTMENT_MASTER = only_one.PK_APPOINTMENT_MASTER WHERE ra.rn = 1 AND ra.DATE BETWEEN $prev_year_date_condition");
                                                    ?>
