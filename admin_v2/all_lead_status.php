...
$header_text = '';
$header_data = $db->Execute("SELECT * FROM `DOA_HEADER_TEXT` WHERE ACTIVE = 1 AND HEADER_TITLE = 'Lead Status page'");
...
foreach ($lead_status_defaults as $key => $value) {
    ...
    if ($is_exist->RecordCount() == 0) {
        ...
        $lead_status_data['DISPLAY_ORDER'] = $counter_default;
...
if ($status_check == 'active') {
    $active_condition = "DOA_LEAD_STATUS.ACTIVE = 1";
} else {
    $active_condition = "DOA_LEAD_STATUS.ACTIVE = 0";
}

// Count total records
$count_query = "SELECT COUNT(*) as total 
                FROM DOA_LEAD_STATUS 
                WHERE DOA_LEAD_STATUS.PK_ACCOUNT_MASTER = " . intval($_SESSION['PK_ACCOUNT_MASTER']) . " 
                AND $active_condition";

if (!empty($search)) {
    $count_query .= " AND DOA_LEAD_STATUS.LEAD_STATUS LIKE '%" . addslashes($search) . "%'";
}
...
// Get lead statuses for current page
$query = "SELECT * FROM DOA_LEAD_STATUS 
          WHERE DOA_LEAD_STATUS.PK_ACCOUNT_MASTER = " . intval($_SESSION['PK_ACCOUNT_MASTER']) . " 
          AND $active_condition";

if (!empty($search)) {
    $query .= " AND DOA_LEAD_STATUS.LEAD_STATUS LIKE '%" . addslashes($search) . "%'";
}

$query .= " ORDER BY DOA_LEAD_STATUS.DISPLAY_ORDER ASC, DOA_LEAD_STATUS.LEAD_STATUS ASC LIMIT $offset, $per_page";
$lead_statuses = $db->Execute($query);
?>
...
<body>
                ...
                <div class="main-card">
                        ...
                        <div>
                            <h2 class="fw-semibold h4 mb-1">
                                <?php if ($status_check == 'inactive') { ?>
                                    <i class="bi bi-slash-circle me-2 text-muted"></i>Not Active Lead Status
                                <?php } else { ?>
                                    <i class="bi bi-check-circle-fill me-2 text-success"></i>Active Lead Status
                                <?php } ?>
                            </h2>
                            <p class="text-muted small mb-0">Manage lead statuses, colors, and display order</p>
                        </div>
                        <div class="d-flex gap-2">
                            <button class="btn btn-success-custom rounded-pill d-flex align-items-center gap-2" onclick="createNewStatus()">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                        <div class="search-container">
                            ...
                            <input type="text" class="form-control search-input" placeholder="Search by status name..." id="searchInput" value="<?= htmlspecialchars($search) ?>">
                        </div>
                    ...
                    <div class="text-muted small mb-3 d-flex align-items-center gap-2">
                        <i class="bi bi-tags"></i> <?= $total_records ?> <?= $total_records == 1 ? 'lead status' : 'lead statuses' ?>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table custom-table align-middle mb-4">
                            ...
                            <tbody>
                                ...
                                $counter = 0;
                                $row_number = $offset + 1;
                                if ($lead_statuses && !$lead_statuses->EOF):
                                    while (!$lead_statuses->EOF):
                                        $PK_LEAD_STATUS = $lead_statuses->fields['PK_LEAD_STATUS'];
                                        $lead_status = $lead_statuses->fields['LEAD_STATUS'];
                                        $status_color = $lead_statuses->fields['STATUS_COLOR'] ?? '#6c757d';
                                        $display_order = $lead_statuses->fields['DISPLAY_ORDER'] ?? '—';
                                        $is_active = $lead_statuses->fields['ACTIVE'] == 1;

                                        $initials = getStatusInitials($lead_status);
                                        $bg_color = getStatusAvatarColor($counter);

                                        // Determine if color is light for text contrast
                                        $is_light_color = isLightColor($status_color);
                                ?>
                                        <tr>
                                            ...
                                            <td>
                                                <div class="action-icons">
                                                    <a href="javascript:;" onclick="editStatus(<?= $PK_LEAD_STATUS ?>);" title="Edit">
                                ...
                                if ($total_records == 0):
                                    ...
                                    <tr>
                                        <td colspan="6" class="text-center py-5">
                                            ...
                                            <p class="mt-3 text-muted">No lead statuses found for the selected filters</p>
                                            <button class="btn btn-sm btn-outline-primary mt-2" onclick="createNewStatus()">Create your first lead status</button>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
    ...
    <?php
    // Helper functions
    function getStatusInitials($name)
    {
    ...
    function getStatusAvatarColor($index)
    {
        $gradients = [
