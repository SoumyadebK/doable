...
<h6 class="mb-2" style="color: #f44336;">Enrollment Details</h6>
<table id="myTable" class="table billing_details">
    <thead style="cursor:pointer;" onclick="$(this).next().slideToggle();">
        <tr>
            <th style="text-align: left; padding-left: 32px;">Due Date</th>
            ...
            if ($payment_details->RecordCount() > 0) {
                ...
                while (!$payment_details->EOF) {
                    ...
                    <tr style="border-style: hidden; color: <?= ($payment_details->fields['TYPE'] == 'Refund') ? 'green' : '' ?>; background-color: <?= (fmod($b, 2) == 0) ? '#ebeced' : '' ?>;">
                        ...
                        <td style="text-align: right; padding-right: 45px;">
                            <?php if (($payment_details->fields['IS_REFUNDED'] == 0) && ($billing_details->fields['STATUS'] == 'A') && (in_array((int)$_SESSION['PK_ROLES'], [2, 3, 11])) && ($payment_details->fields['TYPE'] == 'Payment')) { ?>
                                <a class="btn btn-secondary <?= ($payment_details->fields['IS_REFUNDED'] == 1) ? 'disabled' : '' ?>" href="javascript:" onclick="moveToWallet(this, <?= $payment_details->fields['PK_ENROLLMENT_PAYMENT'] ?>, <?= $payment_details->fields['PK_ENROLLMENT_MASTER'] ?>, <?= $payment_details->fields['PK_ENROLLMENT_LEDGER'] ?>, <?= $PK_USER_MASTER ?>, <?= $payment_details->fields['AMOUNT'] ?>, 'active', 'Move', <?= $p ?>)">Move</a>
                                <a class="btn btn-secondary <?= ($payment_details->fields['IS_REFUNDED'] == 1) ? 'disabled' : '' ?>" href="javascript:" onclick="moveToWallet(this, <?= $payment_details->fields['PK_ENROLLMENT_PAYMENT'] ?>, <?= $payment_details->fields['PK_ENROLLMENT_MASTER'] ?>, <?= $payment_details->fields['PK_ENROLLMENT_LEDGER'] ?>, <?= $PK_USER_MASTER ?>, <?= $payment_details->fields['AMOUNT'] ?>, 'active', 'Refund', <?= $p ?>)">Refund</a>
                            <?php } /* elseif (($payment_details->fields['IS_REFUNDED'] == 1 && $billing_details->fields['AMOUNT_REMAIN'] > 0) && ($billing_details->fields['STATUS'] == 'A' || in_array((int)$_SESSION['PK_ROLES'], [2, 3, 11])) && ($payment_details->fields['TYPE'] == 'Payment')) { ?>
...
<h6 class="mb-2" style="color: #1E90FF;">Appointment Details</h6>
<table id="myTable" class="table border appointment_details">
    <thead style="cursor:pointer;" onclick="$(this).closest('.appointment_details').find('tbody').slideToggle();">
        <tr>
            ...
            <th style="text-align: center;"><?= $service_provider_title ?></th>
            ...
            foreach (array_reverse($appointment_array) as $key => $appointment_value) { ?>
                <tr>
                    ...
                    if ($appointment_value['IS_CHARGED'] == 1) { ?>
                        <td style="text-align: center;"><?= $appointment_value['APPOINTMENT_NUMBER'] ?></td>
                    <?php } else { ?>
                        <td></td>
                    <?php } ?>
                    <td style="text-align: center;">
                        <span class="badge-service ms-auto" style="padding: 2px 6px; background-color: <?= $appointment_value['COLOR_CODE']; ?>20 !important; color: <?= $appointment_value['COLOR_CODE']; ?>"><?= $appointment_value['SERVICE_CODE'] ?></span>
                    </td>
                    ...
                    <td style="text-align: center;">
                        <span class="badge-service" style="padding: 2px 6px; margin-left: 5px; background-color: <?= $appointment_value['STATUS_COLOR'] ?>20 !important; color: <?= $appointment_value['STATUS_COLOR'] ?> !important;">
