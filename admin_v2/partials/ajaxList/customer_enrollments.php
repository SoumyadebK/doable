    ...
    <div class="enrollment-container mb-4">
        ...
        while (!$enr_service_data->EOF) {
            if ($enr_service_data->fields['PK_SERVICE_CLASS'] == 5) {
                $misc_balance += $enr_service_data->fields['FINAL_AMOUNT'];
            } else {
                $credit_balance += $enr_service_data->fields['FINAL_AMOUNT'];
            }
            ...
            <div class="row align-items-center flex-nowrap gx-2" style="width:100%;">
                ...
                <div class="col-2">
                    <?php if ($AGREEMENT_PDF_LINK != '' && $AGREEMENT_PDF_LINK != null) { ?>
                        <a href="enrollment_agreement.php?id=<?= $PK_ENROLLMENT_MASTER ?>" class="view-schedule text-primary" target="_blank">View Agreement</a><i class="bi bi-envelope-fill" title="Mail to Customer" style="font-size: 18px; color: #39b54a; margin-left: 10px; cursor: pointer;" onclick="mailAgreementToCustomer(<?= $PK_ENROLLMENT_MASTER ?>)"></i>
                    <?php } ?>
                </div>
                <div class="col-2">
