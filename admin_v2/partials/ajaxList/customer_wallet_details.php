...
<div class="payments-card">
    <div class="d-flex justify-content-between align-items-start">
        <div>
            <h5 class="fw-bold mb-1">Wallet Payments</h5>
            <p class="text-muted small mb-0">Optional settings section description</p>
        </div>
        <button class="btn btn-light btn-sm btn-outline-edit border text-muted px-3 py-2" style="border-radius: 8px;" onclick="openWalletModel()">
            <i class="bi bi-plus"></i> Add Money to Wallet
        </button>
    </div>
...
<script>
    ...
    document.addEventListener('click', function(event) {
        const btn = event.target.closest('.btn-receipt');
        if (!btn) return;
