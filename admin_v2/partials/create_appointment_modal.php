                        ...
                        <div class="col-8 col-md-8">
                            <div class="form-group">
                                <select class="form-control" id="PK_SCHEDULING_CODE" name="PK_SCHEDULING_CODE" onchange="getSlotsForAppointmentCreation(this)" required>
                                    <option value="">Select Scheduling Code</option>
                                    <?php
                                    $row = $db_account->Execute("SELECT DOA_SCHEDULING_CODE.`PK_SCHEDULING_CODE`, DOA_SCHEDULING_CODE.`SCHEDULING_CODE`, DOA_SCHEDULING_CODE.`SCHEDULING_NAME`, DOA_SCHEDULING_CODE.`DURATION`, DOA_SCHEDULING_CODE.`FOR_RECORD_ONLY` FROM `DOA_SCHEDULING_CODE` WHERE DOA_SCHEDULING_CODE.PK_LOCATION IN (" . $DEFAULT_LOCATION_ID . ") AND DOA_SCHEDULING_CODE.`ACTIVE` = 1 AND DOA_SCHEDULING_CODE.`FOR_RECORD_ONLY` = 1 ORDER BY CASE WHEN DOA_SCHEDULING_CODE.SORT_ORDER IS NULL THEN 1 ELSE 0 END, DOA_SCHEDULING_CODE.SORT_ORDER");
                                    while (!$row->EOF) { ?>
