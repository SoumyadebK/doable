...
if ($REPEAT == 'Daily') {
    ...
    while ($APPOINTMENT_START_DATE <= $APPOINTMENT_END_DATE) {
        $APPOINTMENT_DATE_ARRAY[] = $APPOINTMENT_START_DATE;
...
if ($REPEAT == 'Weekly') {
    ...
    $TOTAL_APPOINTMENT_TO_CREATE = (isset($_POST['OCCURRENCE_AFTER']) && $REPEAT == 'Custom') ? $_POST['OCCURRENCE_AFTER'] : count($APPOINTMENT_DATE_ARRAY);
}
...
for ($n = 0; $n < $TOTAL_APPOINTMENT_TO_CREATE; $n++) {
    ...
    for ($i = 0; $i < count($START_TIME_ARRAY); $i++) {
        if (($i < $SESSION_LEFT) && ($PK_ENROLLMENT_MASTER > 0) && ($PK_ENROLLMENT_SERVICE > 0)) {
...
markFollowupArchived($PK_USER_MASTER, 'NO_FUTURE_APPOINTMENTS');
