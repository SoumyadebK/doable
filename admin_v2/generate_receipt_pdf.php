...
<body>
    ...
    <div id="printContent">
        ...
        if ($PK_ENROLLMENT_MASTER > 0) {
            $enrollment_payment = $db_account->Execute("SELECT DOA_ENROLLMENT_PAYMENT.*, DOA_PAYMENT_TYPE.PAYMENT_TYPE, DOA_ENROLLMENT_TIP.TIP_PERCENTAGE, DOA_ENROLLMENT_TIP.TIP_AMOUNT FROM DOA_ENROLLMENT_PAYMENT LEFT JOIN $master_database.DOA_PAYMENT_TYPE AS DOA_PAYMENT_TYPE ON DOA_ENROLLMENT_PAYMENT.PK_PAYMENT_TYPE = DOA_PAYMENT_TYPE.PK_PAYMENT_TYPE LEFT JOIN DOA_ENROLLMENT_TIP ON DOA_ENROLLMENT_PAYMENT.PK_ENROLLMENT_MASTER = DOA_ENROLLMENT_TIP.PK_ENROLLMENT_MASTER WHERE DOA_ENROLLMENT_PAYMENT.PK_ENROLLMENT_MASTER = '$PK_ENROLLMENT_MASTER' AND DOA_ENROLLMENT_PAYMENT.IS_ORIGINAL_RECEIPT = 1 AND RECEIPT_NUMBER = '$RECEIPT_NUMBER'");
        }

        if ($PK_ENROLLMENT_MASTER <= 0 || $enrollment_payment->RecordCount() == 0) {
            $enrollment_payment = $db_account->Execute("SELECT DOA_ENROLLMENT_PAYMENT.*, DOA_PAYMENT_TYPE.PAYMENT_TYPE, DOA_ENROLLMENT_TIP.TIP_PERCENTAGE, DOA_ENROLLMENT_TIP.TIP_AMOUNT FROM DOA_ENROLLMENT_PAYMENT LEFT JOIN $master_database.DOA_PAYMENT_TYPE AS DOA_PAYMENT_TYPE ON DOA_ENROLLMENT_PAYMENT.PK_PAYMENT_TYPE = DOA_PAYMENT_TYPE.PK_PAYMENT_TYPE LEFT JOIN DOA_ENROLLMENT_TIP ON DOA_ENROLLMENT_PAYMENT.PK_ENROLLMENT_MASTER = DOA_ENROLLMENT_TIP.PK_ENROLLMENT_MASTER WHERE (DOA_ENROLLMENT_PAYMENT.PK_ENROLLMENT_MASTER = 0 && DOA_ENROLLMENT_PAYMENT.TYPE = 'Wallet') AND DOA_ENROLLMENT_PAYMENT.IS_ORIGINAL_RECEIPT = 1 AND RECEIPT_NUMBER = '$RECEIPT_NUMBER'");
        }
        ...
        if ($enrollment_payment->RecordCount() > 0) {
            while (!$enrollment_payment->EOF) {
                ...
                $TIPS_PERCENTAGE = ($enrollment_payment->fields['TIP_PERCENTAGE']) ? number_format($enrollment_payment->fields['TIP_PERCENTAGE'], 2) . '%' : '0%';
                $TIPS_AMOUNT = ($enrollment_payment->fields['TIP_AMOUNT']) ? '$' . number_format($enrollment_payment->fields['TIP_AMOUNT'], 2) : '$0.00';
