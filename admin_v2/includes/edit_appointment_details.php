...
if ($APPOINTMENT_TYPE == 'GROUP') {
    $appointment_enr_data = $db_account->Execute("SELECT * FROM DOA_APPOINTMENT_ENROLLMENT WHERE PK_APPOINTMENT_MASTER = '$PK_APPOINTMENT_MASTER' AND PK_USER_MASTER = '$PK_USER_MASTER'");
...
<div class="modal-dialog">
    <form method="post" action="includes/update_appointment_details.php" enctype="multipart/form-data">
        <input type="hidden" name="REDIRECT_URL" value="../customer.php?id=<?= $PK_USER ?>&master_id=<?= $PK_USER_MASTER ?>#enrollments">
        <input type="hidden" name="PK_APPOINTMENT_MASTER" class="PK_APPOINTMENT_MASTER" value="<?= $PK_APPOINTMENT_MASTER ?>">
        <input type="hidden" name="PK_USER_MASTER" class="PK_USER_MASTER" value="<?= $PK_USER_MASTER ?>">
