...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete' && csrf_check()) {
  try {
    ...
    error_log('blog delete failed: ' . $ex->getMessage());
  }
...
$posts = [];
try {
  $posts = db()->query('SELECT id, title, slug, category, published, published_at, updated_at FROM blog_posts ORDER BY updated_at DESC')->fetchAll();
} catch (Throwable $ex) {
