															...
															<div class="row pt-2" style="margin-bottom:10px;">
																...
																<div class="span9 col-md-10">
																	<select name="RECEPTION[]" id="RECEPTION" class="form-control required-entry select2" style="width:95%" multiple required>
																		...
																		<? $res_type = $res_type = $db->Execute("select DOA_USERS.PK_USER, DOA_USERS.USER_NAME, DOA_USERS.FIRST_NAME, DOA_USERS.LAST_NAME from DOA_USERS LEFT JOIN DOA_USER_ROLES ON DOA_USER_ROLES.PK_USER = DOA_USERS.PK_USER WHERE DOA_USERS.ACTIVE = '1' AND DOA_USER_ROLES.PK_ROLES IN (3, 5) AND DOA_USERS.PK_ACCOUNT_MASTER = $PK_ACCOUNT_MASTER AND DOA_USERS.PK_USER != $_SESSION[PK_USER] OR DOA_USERS.PK_USER IN (" . implode(',', $replay_user_array) . ")");
																		while (!$res_type->EOF) {
