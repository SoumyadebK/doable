...
while (!$all_location->EOF) {
    try {
        ...
        $enrollment_data_all = $db1->Execute("SELECT * FROM DOA_ENROLLMENT_MASTER INNER JOIN DOA_ENROLLMENT_BILLING ON DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER = DOA_ENROLLMENT_BILLING.PK_ENROLLMENT_MASTER RIGHT JOIN DOA_ENROLLMENT_LEDGER ON DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER = DOA_ENROLLMENT_LEDGER.PK_ENROLLMENT_MASTER WHERE DOA_ENROLLMENT_MASTER.STATUS = 'A' AND DOA_ENROLLMENT_MASTER.PK_LOCATION = '$PK_LOCATION' AND (DOA_ENROLLMENT_BILLING.PAYMENT_METHOD = 'Payment Plans' OR DOA_ENROLLMENT_BILLING.PAYMENT_METHOD = 'Flexible Payments') AND DOA_ENROLLMENT_LEDGER.DUE_DATE = '" . date('Y-m-d') . "' AND DOA_ENROLLMENT_LEDGER.IS_PAID = 0");
        while (!$enrollment_data_all->EOF) {
            $PAYMENT_STATUS = 'Failed';
            $PAYMENT_INFO_JSON = '';
            $PK_USER_MASTER = $enrollment_data_all->fields['PK_USER_MASTER'];
            $AMOUNT_TO_PAY = $TOTAL_AMOUNT_PAID = ($enrollment_data_all->fields['AMOUNT_REMAIN'] > 0) ? $enrollment_data_all->fields['AMOUNT_REMAIN'] : $enrollment_data_all->fields['BILLED_AMOUNT'];

            $PK_ENROLLMENT_MASTER = $enrollment_data_all->fields['PK_ENROLLMENT_MASTER'];
            $PK_ENROLLMENT_BILLING = $enrollment_data_all->fields['PK_ENROLLMENT_BILLING'];
            $PK_ENROLLMENT_LEDGER = $enrollment_data_all->fields['PK_ENROLLMENT_LEDGER'];

            $RECEIPT_NUMBER_ORIGINAL = generateReceiptNumber($PK_ENROLLMENT_MASTER);

            $user_master = $db->Execute("SELECT DOA_USERS.PK_USER, DOA_USERS.EMAIL_ID, DOA_USERS.FIRST_NAME, DOA_USERS.LAST_NAME, DOA_USERS.PHONE FROM `DOA_USERS` LEFT JOIN DOA_USER_MASTER ON DOA_USERS.PK_USER=DOA_USER_MASTER.PK_USER WHERE DOA_USER_MASTER.PK_USER_MASTER = '$PK_USER_MASTER'");
            $customer_payment_info = $db1->Execute("SELECT CUSTOMER_PAYMENT_ID FROM DOA_CUSTOMER_PAYMENT_INFO WHERE PAYMENT_TYPE = '$PAYMENT_GATEWAY' AND PK_USER = " . $user_master->fields['PK_USER']);
            ...
            if ($customer_payment_info->RecordCount() > 0) {
                ...
                if ($PAYMENT_GATEWAY == 'Stripe') {
                    ...
                    try {
                        ...
                        $PAYMENT_METHOD_ID = $all_cards->data[0]->id;
                    ...
                    try {
                        ...
                        $PAYMENT_METHOD_ID = $all_cards[0]->getId();
                    ...
                    try {
                        ...
                        $request->setMerchantAuthentication($merchantAuthentication);
                        $request->setCustomerProfileId($CUSTOMER_PAYMENT_ID);
                        ...
                        $PAYMENT_METHOD_ID = $paymentProfiles[0]->getCustomerPaymentProfileId();
                ...
                if ($PAYMENT_STATUS == 'Success') {
                    $enrollmentServiceData = $db1->Execute("SELECT * FROM `DOA_ENROLLMENT_SERVICE` WHERE `PK_ENROLLMENT_MASTER` = " . $PK_ENROLLMENT_MASTER);
                    $ACTUAL_AMOUNT = $enrollment_data_all->fields['TOTAL_AMOUNT'];
                    while (!$enrollmentServiceData->EOF) {
                        if ($enrollmentServiceData->fields['FINAL_AMOUNT'] > 0 && $ACTUAL_AMOUNT > 0) {
                            $servicePercent = ($enrollmentServiceData->fields['FINAL_AMOUNT'] * 100) / $ACTUAL_AMOUNT;
                            $serviceAmount = ($TOTAL_AMOUNT_PAID * $servicePercent) / 100;
