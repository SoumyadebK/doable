...
while (!$row->EOF) {
    $primary_location_id = $row->fields['PK_LOCATION']; ?>
...
<?php
$row = $db->Execute("SELECT DISTINCT DOA_LOCATION.PK_LOCATION, DOA_LOCATION.LOCATION_NAME FROM DOA_LOCATION LEFT JOIN DOA_USER_LOCATION ON DOA_USER_LOCATION.PK_LOCATION = DOA_LOCATION.PK_LOCATION WHERE DOA_USER_LOCATION.PK_LOCATION != $primary_location_id AND DOA_USER_LOCATION.PK_USER = " . $_POST['PK_USER'] . " AND PK_ACCOUNT_MASTER = '$_SESSION[PK_ACCOUNT_MASTER]' AND ACTIVE = 1 ORDER BY DOA_LOCATION.LOCATION_NAME");
while (!$row->EOF) { ?>
    <option value="<?= $row->fields['PK_LOCATION']; ?>" data-location_id="<?= $row->fields['PK_LOCATION'] ?>" <?= ($LOCATION_ID == $row->fields['PK_LOCATION']) ? 'selected' : '' ?>><?= $row->fields['LOCATION_NAME'] ?></option>
<?php $row->MoveNext();
} ?>