...
while (!$row->EOF) { ?>
    <option value="<?= $row->fields['PK_LOCATION']; ?>" <?= (!empty($_SESSION['MIGRATION_LOCATION']) && $_SESSION['MIGRATION_LOCATION'] == $row->fields['PK_LOCATION']) ? 'selected' : '' ?>><?= $row->fields['LOCATION_NAME'] ?></option>
<?php $row->MoveNext();
} ?>