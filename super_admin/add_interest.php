...
if(!empty($_POST)){
    ...
    if(empty($_GET['id'])){
        ...
        db_perform('DOA_INTERESTS', $INTEREST_DATA, 'insert');
    }else{
        $INTEREST_DATA['ACTIVE'] = $_POST['ACTIVE'];
        $INTEREST_DATA['EDITED_BY'] = $_SESSION['PK_USER'];
        $INTEREST_DATA['EDITED_ON'] = date("Y-m-d H:i");
        db_perform('DOA_INTERESTS', $INTEREST_DATA, 'update'," PK_INTERESTS =  '$_GET[id]'");
    }
...
if(empty($_GET['id'])){
    ...
    $res = $db->Execute("SELECT * FROM DOA_INTERESTS WHERE PK_INTERESTS = '$_GET[id]'");
