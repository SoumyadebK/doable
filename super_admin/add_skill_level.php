...
if(!empty($_POST)){
    ...
    if(empty($_GET['id'])){
        ...
        db_perform('DOA_SKILL_LEVEL', $SKILL_LEVEL_DATA, 'insert');
    }else{
        $SKILL_LEVEL_DATA['ACTIVE'] = $_POST['ACTIVE'];
        $SKILL_LEVEL_DATA['EDITED_BY'] = $_SESSION['PK_USER'];
        $SKILL_LEVEL_DATA['EDITED_ON'] = date("Y-m-d H:i");
        db_perform('DOA_SKILL_LEVEL', $SKILL_LEVEL_DATA, 'update'," PK_SKILL_LEVEL =  '$_GET[id]'");
    }
...
if(empty($_GET['id'])){
    ...
    $res = $db->Execute("SELECT * FROM `DOA_SKILL_LEVEL` WHERE PK_SKILL_LEVEL = '$_GET[id]'");
