...
function CalendarPicker(element, options) {
    // Core variables.
    //this.date = new Date();
	this.date = options.default_date;
	...
	this.disabled_days = options.disabled_days;
...
CalendarPicker.prototype._insertDaysIntoGrid = function () {
    ...
    arrayOfDays.forEach(date => {
		var month1 = date.toString().split(' ')[1];
		if(month1 == 'Jan')
			month1 = '01'
		else if(month1 == 'Feb')
			month1 = '02'
		...
		var date2 = date.toString().split(' ')[3]+'-'+month1+'-'+date.toString().split(' ')[2];
		//alert(date+"\n"+Date+"\n"+date2+"\n")
        ...
        if (dateIsBetweenAllowedRange || this.disabled_days.includes(date2) ) {
