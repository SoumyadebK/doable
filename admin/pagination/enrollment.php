...
<table  class="table table-striped border" data-page-length='50'>
    ...
    $row = $db->Execute("SELECT $account_database.DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER, $account_database.DOA_ENROLLMENT_MASTER.ENROLLMENT_ID, $account_database.DOA_ENROLLMENT_MASTER.ACTIVE, $account_database.DOA_ENROLLMENT_MASTER.STATUS, $account_database.DOA_ENROLLMENT_MASTER.PK_USER_MASTER, $master_database.DOA_USERS.FIRST_NAME, $master_database.DOA_USERS.LAST_NAME, $master_database.DOA_USERS.EMAIL_ID, $master_database.DOA_USERS.PHONE, $master_database.DOA_LOCATION.LOCATION_NAME, $account_database.DOA_SERVICE_MASTER.SERVICE_NAME, $account_database.DOA_SERVICE_CODE.SERVICE_CODE FROM $account_database.`DOA_ENROLLMENT_MASTER` INNER JOIN $master_database.DOA_USER_MASTER ON $account_database.DOA_ENROLLMENT_MASTER.PK_USER_MASTER = $master_database.DOA_USER_MASTER.PK_USER_MASTER INNER JOIN $master_database.DOA_USERS ON $master_database.DOA_USERS.PK_USER = $master_database.DOA_USER_MASTER.PK_USER RIGHT JOIN $account_database.DOA_ENROLLMENT_SERVICE ON $account_database.DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER = $account_database.DOA_ENROLLMENT_SERVICE.PK_ENROLLMENT_MASTER LEFT JOIN $account_database.DOA_SERVICE_MASTER ON $account_database.DOA_ENROLLMENT_SERVICE.PK_SERVICE_MASTER = $account_database.DOA_SERVICE_MASTER.PK_SERVICE_MASTER LEFT JOIN $account_database.DOA_SERVICE_CODE ON $account_database.DOA_ENROLLMENT_SERVICE.PK_SERVICE_CODE = $account_database.DOA_SERVICE_CODE.PK_SERVICE_CODE LEFT JOIN $master_database.DOA_LOCATION ON $master_database.DOA_LOCATION.PK_LOCATION = $account_database.DOA_ENROLLMENT_MASTER.PK_LOCATION WHERE $master_database.DOA_USER_MASTER.PRIMARY_LOCATION_ID IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND $account_database.DOA_ENROLLMENT_MASTER.PK_USER_MASTER='$_GET[master_id]' AND $account_database.DOA_ENROLLMENT_MASTER.PK_ACCOUNT_MASTER=".$_SESSION['PK_ACCOUNT_MASTER'].$search." ORDER BY $account_database.DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER DESC LIMIT " . $page_first_result . ',' . $results_per_page);
    while (!$row->EOF) { ?>
        <tr>
            <td onclick="editpage(<?=$row->fields['PK_ENROLLMENT_MASTER']?>);"><?=$i;?></td>
            <td onclick="editpage(<?=$row->fields['PK_ENROLLMENT_MASTER']?>);"><?=$row->fields['ENROLLMENT_ID']." || ".$row->fields['SERVICE_NAME']." || ".$row->fields['SERVICE_CODE']?></td>
