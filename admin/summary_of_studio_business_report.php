                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                ...
                                <table id="myTable" class="table table-bordered" data-page-length='50'>
                                    ...
                                    <?php
                                    $weekly_pre_original_units = $db_account->Execute("SELECT SUM(NUMBER_OF_SESSION) AS UNITS, SUM(ORIGINAL_SESSION_COUNT) AS ORIGINAL_UNITS, SUM(FINAL_AMOUNT) AS AMOUNT, SUM(ORIGINAL_AMOUNT) AS ORIGINAL_AMOUNT FROM `DOA_ENROLLMENT_SERVICE` LEFT JOIN DOA_ENROLLMENT_MASTER ON DOA_ENROLLMENT_SERVICE.PK_ENROLLMENT_MASTER = DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER WHERE DOA_ENROLLMENT_SERVICE.STATUS IN ('C', 'CA') AND PK_ENROLLMENT_TYPE = 5 AND ENROLLMENT_DATE BETWEEN $weekly_date_condition");
                                    $weekly_original_units = $db_account->Execute("SELECT SUM(NUMBER_OF_SESSION) AS UNITS, SUM(ORIGINAL_SESSION_COUNT) AS ORIGINAL_UNITS, SUM(FINAL_AMOUNT) AS AMOUNT, SUM(ORIGINAL_AMOUNT) AS ORIGINAL_AMOUNT FROM `DOA_ENROLLMENT_SERVICE` LEFT JOIN DOA_ENROLLMENT_MASTER ON DOA_ENROLLMENT_SERVICE.PK_ENROLLMENT_MASTER = DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER WHERE DOA_ENROLLMENT_SERVICE.STATUS IN ('C', 'CA') AND PK_ENROLLMENT_TYPE = 2 AND ENROLLMENT_DATE BETWEEN $weekly_date_condition");
                                    $weekly_extension_units = $db_account->Execute("SELECT SUM(NUMBER_OF_SESSION) AS UNITS, SUM(ORIGINAL_SESSION_COUNT) AS ORIGINAL_UNITS, SUM(FINAL_AMOUNT) AS AMOUNT, SUM(ORIGINAL_AMOUNT) AS ORIGINAL_AMOUNT FROM `DOA_ENROLLMENT_SERVICE` LEFT JOIN DOA_ENROLLMENT_MASTER ON DOA_ENROLLMENT_SERVICE.PK_ENROLLMENT_MASTER = DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER WHERE DOA_ENROLLMENT_SERVICE.STATUS IN ('C', 'CA') AND PK_ENROLLMENT_TYPE = 9 AND ENROLLMENT_DATE BETWEEN $weekly_date_condition");
                                    $weekly_renewal_units = $db_account->Execute("SELECT SUM(NUMBER_OF_SESSION) AS UNITS, SUM(ORIGINAL_SESSION_COUNT) AS ORIGINAL_UNITS, SUM(FINAL_AMOUNT) AS AMOUNT, SUM(ORIGINAL_AMOUNT) AS ORIGINAL_AMOUNT FROM `DOA_ENROLLMENT_SERVICE` LEFT JOIN DOA_ENROLLMENT_MASTER ON DOA_ENROLLMENT_SERVICE.PK_ENROLLMENT_MASTER = DOA_ENROLLMENT_MASTER.PK_ENROLLMENT_MASTER WHERE DOA_ENROLLMENT_SERVICE.STATUS IN ('C', 'CA') AND PK_ENROLLMENT_TYPE = 13 AND ENROLLMENT_DATE BETWEEN $weekly_date_condition");
                                    ?>
                                    <tr>
                                        <th style="width:9%; text-align: center;font-weight: bold">Adjust</th>
                                        <th style="width:18%; text-align: center; font-weight: normal !important" colspan="2"><?=$wpo_unit=$weekly_pre_original_units->fields['ORIGINAL_UNITS']-$weekly_pre_original_units->fields['UNITS']?> / <?=$wpo_session_price=number_format($weekly_pre_original_units->fields['ORIGINAL_AMOUNT']/$weekly_pre_original_units->fields['ORIGINAL_UNITS'], 2)?> / $<?=$wpo_total = number_format(($weekly_pre_original_units->fields['ORIGINAL_UNITS']-$weekly_pre_original_units->fields['UNITS'])*($weekly_pre_original_units->fields['ORIGINAL_AMOUNT']/$weekly_pre_original_units->fields['ORIGINAL_UNITS']), 2)?></th>
                                        <th style="width:18%; text-align: center; font-weight: normal !important" colspan="2"><?=$wo_unit=$weekly_original_units->fields['ORIGINAL_UNITS']-$weekly_original_units->fields['UNITS']?> / <?=$wo_session_price=number_format($weekly_original_units->fields['ORIGINAL_AMOUNT']/$weekly_original_units->fields['ORIGINAL_UNITS'], 2)?> / $<?=$wo_total=number_format(($weekly_original_units->fields['ORIGINAL_UNITS']-$weekly_original_units->fields['UNITS'])*($weekly_original_units->fields['ORIGINAL_AMOUNT']/$weekly_original_units->fields['ORIGINAL_UNITS']), 2)?></th>
                                        <th style="width:18%; text-align: center; font-weight: normal !important" colspan="2"><?=$we_unit=$weekly_extension_units->fields['ORIGINAL_UNITS']-$weekly_extension_units->fields['UNITS']?> / <?=$we_session_price=number_format($weekly_extension_units->fields['ORIGINAL_AMOUNT']/$weekly_extension_units->fields['ORIGINAL_UNITS'], 2)?> / $<?=$we_total=number_format(($weekly_extension_units->fields['ORIGINAL_UNITS']-$weekly_extension_units->fields['UNITS'])*($weekly_extension_units->fields['ORIGINAL_AMOUNT']/$weekly_extension_units->fields['ORIGINAL_UNITS']), 2)?></th>
                                        <th style="width:18%; text-align: center; font-weight: normal !important" colspan="2"><?=$wr_unit=$weekly_renewal_units->fields['ORIGINAL_UNITS']-$weekly_renewal_units->fields['UNITS']?> / <?=$wr_session_price=number_format($weekly_renewal_units->fields['ORIGINAL_AMOUNT']/$weekly_renewal_units->fields['ORIGINAL_UNITS'], 2)?> / $<?=$wr_total=number_format(($weekly_renewal_units->fields['ORIGINAL_UNITS']-$weekly_renewal_units->fields['UNITS'])*($weekly_renewal_units->fields['ORIGINAL_AMOUNT']/$weekly_renewal_units->fields['ORIGINAL_UNITS']), 2)?></th>
                                        <th style="width:18%; text-align: center; font-weight: normal !important" colspan="2"><?=$wpo_unit+$wo_unit+$we_unit+$wr_unit?> / <?=number_format((float)($wpo_session_price)+(float)$wo_session_price+(float)$we_session_price+(float)$wr_session_price, 2)?>/ $<?=number_format((float)($wpo_total)+(float)$wo_total+(float)$we_total+(float)$wr_total, 2)?></th>
                                    </tr>
