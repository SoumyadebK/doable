                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table id="myTable" class="table table-bordered" data-page-length='50'>
                                    ...
                                    <tr>
                                        ...
                                        $upto_three_ytd_data = $db_account->Execute("SELECT SUM(CUSTOMER_COUNT) as TOTAL_CUSTOMER FROM (SELECT COUNT(DISTINCT PK_USER_MASTER) AS CUSTOMER_COUNT FROM DOA_APPOINTMENT_MASTER LEFT JOIN DOA_APPOINTMENT_CUSTOMER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER=DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_MASTER.PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND APPOINTMENT_TYPE='NORMAL' AND DOA_APPOINTMENT_MASTER.CREATED_ON BETWEEN DATE_FORMAT(CURDATE(), '%Y-01-01') AND '".date('Y-m-d', strtotime($to_date))."' GROUP BY PK_USER_MASTER HAVING COUNT(DISTINCT DOA_APPOINTMENT_MASTER.PK_ENROLLMENT_MASTER) <= 3) as TOTAL_CUSTOMER");
                                        $upto_three_ytd = $upto_three_ytd_data->fields['TOTAL_CUSTOMER'] > 0 ? $upto_three_ytd_data->fields['TOTAL_CUSTOMER'] : '0';
                                        $above_three_ytd_data = $db_account->Execute("SELECT SUM(CUSTOMER_COUNT) as TOTAL_CUSTOMER FROM (SELECT COUNT(DISTINCT PK_USER_MASTER) AS CUSTOMER_COUNT FROM DOA_APPOINTMENT_MASTER LEFT JOIN DOA_APPOINTMENT_CUSTOMER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER=DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_MASTER.PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND APPOINTMENT_TYPE='NORMAL' AND DOA_APPOINTMENT_MASTER.CREATED_ON BETWEEN DATE_FORMAT(CURDATE(), '%Y-01-01') AND '".date('Y-m-d', strtotime($to_date))."' GROUP BY PK_USER_MASTER HAVING COUNT(DISTINCT DOA_APPOINTMENT_MASTER.PK_ENROLLMENT_MASTER) > 3) as TOTAL_CUSTOMER");
                                        $above_three_ytd = $above_three_ytd_data->fields['TOTAL_CUSTOMER'] > 0 ? $above_three_ytd_data->fields['TOTAL_CUSTOMER'] : '0';
                                        $group_class_ytd_data = $db_account->Execute("SELECT COUNT(DOA_APPOINTMENT_CUSTOMER.PK_USER_MASTER) AS GROUP_CLASS FROM `DOA_APPOINTMENT_MASTER` LEFT JOIN DOA_APPOINTMENT_CUSTOMER ON DOA_APPOINTMENT_MASTER.PK_APPOINTMENT_MASTER=DOA_APPOINTMENT_CUSTOMER.PK_APPOINTMENT_MASTER WHERE DOA_APPOINTMENT_MASTER.PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND APPOINTMENT_TYPE='GROUP' AND DOA_APPOINTMENT_MASTER.CREATED_ON BETWEEN DATE_FORMAT(CURDATE(), '%Y-01-01') AND '".date('Y-m-d', strtotime($to_date))."'");
                            ...
                            <div class="table-responsive">
                                ...
                                <table id="myTable" class="table table-bordered" data-page-length='50'>
                                    ...
                                    $t2_data = $db_account->Execute("SELECT COUNT(DISTINCT DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS T_2 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 2 AND CREATED_ON BETWEEN '".date('Y-m-d', strtotime($from_date))."' AND '".date('Y-m-d', strtotime($to_date))."'");
                                    ...
                                    if ($t1_completed_data->fields['T_1'] > 0) {
                                        $t2 = $t2_data->fields['T_2']+1;
                                    } else {
                                        $t2 = $t2_data->fields['T_2'] > 0 ? $t2_data->fields['T_2'] : 0;
                                    }

                                    $t3_data = $db_account->Execute("SELECT COUNT(DISTINCT DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS T_3 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 3 AND CREATED_ON BETWEEN '".date('Y-m-d', strtotime($from_date))."' AND '".date('Y-m-d', strtotime($to_date))."'");
                                    ...
                                    if ($t3_completed_data->fields['T_3'] > 0) {
                                        $t3 = $t3_completed_data->fields['T_3']+1;
                                    } else {
                                        $t3 = $t3_data->fields['T_3'] > 0 ? $t3_data->fields['T_3'] : 0;
                                    }

                                    $t4_data = $db_account->Execute("SELECT COUNT(DISTINCT DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS T_4 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 4 AND CREATED_ON BETWEEN '".date('Y-m-d', strtotime($from_date))."' AND '".date('Y-m-d', strtotime($to_date))."'");
                                    ...
                                    if ($t4_completed_data->fields['T_4'] > 0) {
                                        $t4 = $t4_completed_data->fields['T_4']+1;
                                    } else {
                                    ...
                                    $s1_data = $db_account->Execute("SELECT COUNT(DISTINCT DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS S_1 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 1 AND ALL_APPOINTMENT_DONE = 0 AND CREATED_ON BETWEEN '".date('Y-m-d', strtotime($from_date))."' AND '".date('Y-m-d', strtotime($to_date))."'");
                                    ...
                                    <tr>
                                        ...
                                        $s2_net_data = $db_account->Execute("SELECT COUNT(DISTINCT DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS S_2 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 3 AND ALL_APPOINTMENT_DONE = 0 AND CREATED_ON BETWEEN DATE_FORMAT(CURDATE(), '%Y-01-01') AND '".date('Y-m-d', strtotime($to_date))."'");
                                        $s3_net_data = $db_account->Execute("SELECT COUNT(DISTINCT DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS S_3 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 4 AND ALL_APPOINTMENT_DONE = 0 AND CREATED_ON BETWEEN DATE_FORMAT(CURDATE(), '%Y-01-01') AND '".date('Y-m-d', strtotime($to_date))."'");
                                        $s4_net_data = $db_account->Execute("SELECT COUNT(DISTINCT DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS S_4 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 5 AND ALL_APPOINTMENT_DONE = 0 AND CREATED_ON BETWEEN DATE_FORMAT(CURDATE(), '%Y-01-01') AND '".date('Y-m-d', strtotime($to_date))."'");
