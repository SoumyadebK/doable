                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                ...
                                <table id="myTable" class="table table-bordered" data-page-length='50'>
                                    ...
                                    <?php
                                    $t1_data = $db_account->Execute("SELECT COUNT(DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS T_1 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 1 AND CREATED_ON BETWEEN '".date('Y-m-d', strtotime($from_date))."' AND '".date('Y-m-d', strtotime($to_date))."'");
                                    ...
                                    $s1_data = $db_account->Execute("SELECT COUNT(DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS S_1 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 1 AND CREATED_ON BETWEEN '".date('Y-m-d', strtotime($from_date))."' AND '".date('Y-m-d', strtotime($to_date))."'");
                                    $s2_data = $db_account->Execute("SELECT COUNT(DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS S_2 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 2 AND CREATED_ON BETWEEN '".date('Y-m-d', strtotime($from_date))."' AND '".date('Y-m-d', strtotime($to_date))."'");
                                    $s3_data = $db_account->Execute("SELECT COUNT(DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS S_3 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 3 AND CREATED_ON BETWEEN '".date('Y-m-d', strtotime($from_date))."' AND '".date('Y-m-d', strtotime($to_date))."'");
                                    $s4_data = $db_account->Execute("SELECT COUNT(DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS S_4 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER >= 4 AND CREATED_ON BETWEEN '".date('Y-m-d', strtotime($from_date))."' AND '".date('Y-m-d', strtotime($to_date))."'");
                                    ...
                                    <tr>
                                        <?php
                                        $t1_net_data = $db_account->Execute("SELECT COUNT(DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS T_1 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 1 AND CREATED_ON BETWEEN DATE_FORMAT(CURDATE(), '%Y-01-01') AND DATE_ADD(CURDATE(), INTERVAL (7 - DAYOFWEEK(CURDATE())) DAY)");
                                        ...
                                        $s1_net_data = $db_account->Execute("SELECT COUNT(DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS S_1 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 1 AND CREATED_ON BETWEEN DATE_FORMAT(CURDATE(), '%Y-01-01') AND '".date('Y-m-d', strtotime($to_date))."'");
                                        $s2_net_data = $db_account->Execute("SELECT COUNT(DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS S_2 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 2 AND CREATED_ON BETWEEN DATE_FORMAT(CURDATE(), '%Y-01-01') AND '".date('Y-m-d', strtotime($to_date))."'");
                                        $s3_net_data = $db_account->Execute("SELECT COUNT(DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS S_3 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER = 3 AND CREATED_ON BETWEEN DATE_FORMAT(CURDATE(), '%Y-01-01') AND '".date('Y-m-d', strtotime($to_date))."'");
                                        $s4_net_data = $db_account->Execute("SELECT COUNT(DOA_ENROLLMENT_MASTER.PK_USER_MASTER) AS S_4 FROM DOA_ENROLLMENT_MASTER WHERE PK_LOCATION IN (".$_SESSION['DEFAULT_LOCATION_ID'].") AND CUSTOMER_ENROLLMENT_NUMBER >= 4 AND CREATED_ON BETWEEN DATE_FORMAT(CURDATE(), '%Y-01-01') AND '".date('Y-m-d', strtotime($to_date))."'");
