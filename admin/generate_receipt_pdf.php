...
foreach ($RECEIPT_NUMBER_ARRAY as $key => $value) {
    if ($PK_ENROLLMENT_MASTER == 0) {
        $wallet_data = $db_account->Execute("SELECT * FROM DOA_CUSTOMER_WALLET WHERE RECEIPT_NUMBER = '$value' ORDER BY PK_CUSTOMER_WALLET DESC LIMIT 1");
        ...
        $enrollment_billing_data = $db_account->Execute("SELECT DOA_ENROLLMENT_BILLING.BILLING_REF, DOA_ENROLLMENT_PAYMENT.PK_ENROLLMENT_MASTER FROM DOA_ENROLLMENT_BILLING LEFT JOIN DOA_ENROLLMENT_PAYMENT ON DOA_ENROLLMENT_BILLING.PK_ENROLLMENT_BILLING = DOA_ENROLLMENT_PAYMENT.PK_ENROLLMENT_BILLING WHERE DOA_ENROLLMENT_PAYMENT.IS_ORIGINAL_RECEIPT = 1 AND DOA_ENROLLMENT_PAYMENT.RECEIPT_NUMBER = '$value' LIMIT 1");
        $BILLING_REF = $enrollment_billing_data->fields['BILLING_REF'];
        $PK_ENROLLMENT_MASTER = $enrollment_billing_data->fields['PK_ENROLLMENT_MASTER'];
    ...
    $enrollment_payment = $db_account->Execute("SELECT DOA_ENROLLMENT_PAYMENT.*, DOA_PAYMENT_TYPE.PAYMENT_TYPE FROM DOA_ENROLLMENT_PAYMENT LEFT JOIN $master_database.DOA_PAYMENT_TYPE AS DOA_PAYMENT_TYPE ON DOA_ENROLLMENT_PAYMENT.PK_PAYMENT_TYPE = DOA_PAYMENT_TYPE.PK_PAYMENT_TYPE WHERE DOA_ENROLLMENT_PAYMENT.TYPE = 'Payment' AND DOA_ENROLLMENT_PAYMENT.IS_ORIGINAL_RECEIPT = 1 AND RECEIPT_NUMBER = ".$value);
...
?>
    ...
    <table style="margin-left: auto; margin-right: auto; width:70%;">
        ...
        <?php } ?>
        <tr>
            <td style="text-align:center"><strong>Receipt# <?=$value?></strong></td>
        </tr>
        <tr>
    ...
    <table style="margin-left: auto; margin-right: auto; width:70%; margin-bottom: 20%;">
        ...
        <tr>
            <td style="text-align:center">*Per authorization on <?=$PAYMENT_DATE?> Receipt# <?=$value?></td>
        </tr>
        </tbody>
    </table>
