...
while (!$row->EOF) { ?>
    <option value="<?php echo $row->fields['PK_LOCATION'];?>"data-location_id="<?=$row->fields['PK_LOCATION']?>" <?=($LOCATION_ID == $row->fields['PK_LOCATION'])?'selected':''?>><?=$row->fields['LOCATION_NAME']?></option>
    <?php $row->MoveNext(); } ?>
