...
<table style="width:100%">
    ...
    if(empty($_POST['PK_ENROLLMENT_PAYMENT'])) {
        ...
        $enrollment_ledger_data = $db_account->Execute("SELECT * FROM DOA_ENROLLMENT_LEDGER WHERE PK_ENROLLMENT_LEDGER=".$PK_ENROLLMENT_LEDGER);
        $enrollment_payment_data = $db_account->Execute("SELECT * FROM DOA_ENROLLMENT_PAYMENT WHERE PK_ENROLLMENT_PAYMENT=".$PK_ENROLLMENT_PAYMENT);
        ...
        $html_template = str_replace('{PAYMENT_METHOD}', $enrollment_payment_data->fields['PK_PAYMENT_TYPE'], $html_template);
        $html_template = str_replace('{CARD_NUMBER}', $enrollment_payment_data->fields['CARD_NUMBER'], $html_template);
                                    ...
                                    <div class="row">
                                        <div class="col-12 d-flex justify-content-end align-items-center" style="font-weight: bold; font-size: 15px; margin-top: 15px;">
                                            ...
                                            while (!$row->EOF) {
                                                ...
                                                {
                                                    $used_session = $db_account->Execute("SELECT COUNT(`PK_ENROLLMENT_MASTER`) AS USED_SESSION_COUNT FROM `DOA_APPOINTMENT_MASTER` WHERE `PK_ENROLLMENT_MASTER` = ".$row->fields['PK_ENROLLMENT_MASTER']." AND PK_SERVICE_CODE = ".$serviceCodeData->fields['PK_SERVICE_CODE']);
